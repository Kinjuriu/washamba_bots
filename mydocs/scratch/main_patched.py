    ...
    if tile is None:
        ...
        if crop:
            if seed_budget is not None:
                ...
                # SCRATCH FIX (v2, matches HANDOFF's failed variant): denied
                # units PASS in place instead of falling through to
                # fertilizer/weed/fallback movement below.
                return ["PASS"]
...
def choose_farmer_action(state, claimed=None, pending_builds=None, feed_claimed=None, seed_budget=None):
...
def nikaangukia_meroni(obs):
    ...
    try:
        ...
        pending_builds = [0]
        feed_claimed = set()
        # SCRATCH FIX (v2 seed budget): mutable per-turn copy of held seeds,
        # decremented as units "win" the right to plant a crop this turn.
        seed_budget = dict(seeds)
        farmer_action = choose_farmer_action(state, claimed, pending_builds, feed_claimed, seed_budget)
        hands_actions = [
            choose_unit_action(
