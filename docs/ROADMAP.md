...
> **Update, 2026-08-17:** `origin/main`'s `da8cdea` (Spidey) added a
> byte-identical copy of this file at the repo root, independently
> verified with 2 more contested-market replay episodes (see
> `docs/REPLAY_ANALYSIS.md`), and merged `3b8d36f` — a cash-reserve retune
> that resolves the §3c sub-additivity caveat below and changes the
> second-sheep numbers in §2. The corrections from that commit message are
> applied below; the root-level copy on `origin/main` does not yet have
> them — reconcile the two copies (or delete one) in a follow-up.
...
**The "Us today" column below is a snapshot from before `3b8d36f`/PR
#13/#14 landed on `origin/main` — treat it as superseded, not current.**
For today's actual numbers read `CLAUDE.md` directly rather than trusting
this table's right-hand column.

| Signal | Observed (6 episodes, Aug 1–16) | Us today (superseded snapshot) |
|---|---|---|
| Final money | $71,757 – $126,015 | $31,132 (self-play) / ~$42k (vs built-ins) |
| `BUY_LAND` | exactly 2×, every episode, always day 6–11 | 0 — **no longer a clean "dead end": every prior test held crew size fixed (see `CLAUDE.md`'s 2026-08-17 correction)** |
| Hands sustained | ramps to 10–14 by day ~8, held through ~day 27 | small fixed cap |
| Animals | COW + SHEEP together (sometimes + GOOSE), every episode | 1 sheep only — **2 is now +900 (8/12) vs `starter` post-`3b8d36f`, was -19,514 (0/12); still not a clean win (t=0.61), see `CLAUDE.md`** |
...
**Resolved, 2026-08-17 (`3b8d36f`):** the sub-additivity was caused by the
hire gate and the old seed-purchase trigger competing for the same days
3-7 trough dollars. Retuning `MIN_CASH_RESERVE_FOR_SEED_BUYING` 100 → 450
removes the trough (seed purchases wait for first-harvest income instead
of spending the starting stake) and the two changes now compose cleanly:
**+4,166 mean, 24/24 head-to-head; +5,389 mean, 10/12, t=3.84 paired vs
`starter`.** Phase 1 can extend the shared-ledger pattern to hiring/wheat
without the composability worry this caveat originally flagged — just
don't assume "higher reserve is always safer": the same commit found a
real interior optimum (450), with both 0-100 and 700+ measurably worse.
...
strategy content, directly reuses a pattern already proven to work. The
sub-additive interaction noted in §3c (a proven pattern meeting an
unrelated hiring change and underperforming either half alone) is now
**resolved** by `3b8d36f`'s cash-reserve retune — still re-measure any new
bundle rather than assuming composability by default, but this specific
caveat is no longer open.
