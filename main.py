...
#
# VARIANT B (sell-or-hold cadence, ported from @Kinjuriu's
# experiment/pricing-cadence-v1 via commit 9a6a5c6): the hard cliff is
# disabled here by design, not as a variant - cadence_urgency() below is
# meant to replace it with a continuous ramp instead of a fixed-day flip.
...
# ---------------------------------------------------------------------
# Sell-or-hold cadence (@Kinjuriu, experiment/pricing-cadence-v1, ported
# onto the Phase 3 base per mydocs/Plan-phase3-aggressive-selling.md
# Variant B)
#
...
def decide_market_actions(
    ...
    for product, quantity in shed.items():
        ...
        # VARIANT B: should_sell()'s fixed per-product floor is no longer on
        # the sell path - estimate_sell_or_hold_value() decides instead,
        # comparing selling now against holding for SELL_HORIZON_DAYS given
        # our own pipeline, the opponent's visible pipeline, and town demand
        # still to arrive. should_sell() itself is kept in the module (tests,
        # and conceptually documents the old behaviour) but not called here.
        if sell_quantity > 0:
