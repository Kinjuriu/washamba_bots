# agents/washamba_base_v1_fork.py - agents/public_farm2945.py byte-verbatim from
# line 3 down, with a two-flag fork overlay appended after it. See the FORK
# OVERLAY banner at the bottom of the file and docs/ENDGAME/fork_results.md.
...
# ===========================================================================
# FORK OVERLAY - washamba_bots, 2026-09-21 (T11).
#
# Reproduces the near-copy fork that beats the base 9-2 on the live ladder
# (11 post-burst episodes of submission 56431632; see
# docs/ENDGAME/ladder_diagnosis_56431632.md and docs/ENDGAME/fork_results.md).
# Everything above this banner is agents/public_farm2945.py byte-verbatim.
#
# The fork's step-0 `BUY_SEED WHEAT 1` is a fingerprint, not the lever: fed to
# the unmodified base it is inert (the seed is never planted). Two real edits
# were recovered from the 11 recorded tapes:
#
#   A. FORK_CA_FEED_DAYS - the `_CA` carrot layer (l.4654-4680) aborts its
#      whole PLANT WHEAT -> PLANT CARROT swap loop whenever
#      `_ca_wheat_total(obs) < _ca_feed_need(seat, step, _CA_FEED_DAYS)`.
#      At the shipped 2 days that gate blocks 48 turns a season on seed
#      633939918 while the shed sits 0-10 units under a feed need of ~34.
#      The fork ends every day from 18 on with LESS wheat than we do and still
#      swaps more, so it is not clearing the gate with stock - it holds a
#      shorter reserve.  `_CA_FEED_DAYS = 1` reproduces the fork's season crop
#      mix on 10 of the 11 episodes (exactly on 9).
#      `V9_CARROT` is not involved: it fires 0 times (needs ratio >= 1.8 AND
#      wheat_held >= 40; the ratio only reaches 1.8 around day 21, by which
#      time the shed holds 31-34).
#
#   B. FORK_OPENING - one extra wheat cycle on tile (2,4) before that tile
#      becomes a pasture: buy 1 wheat seed at step 0, walk an idle hand out
#      and PLANT+WATER it on day 0, water it on days 1-2, harvest it on day 3
#      and only then BUILD_PASTURE, carrying the grain back to the shed.
#      Scripted from the 9-of-11 majority form of the recorded tapes; the
#      other 2 harvest a day earlier (step 53/54 instead of 87/88).
#
# Both flags off => byte-identical behaviour to agents/public_farm2945.py.
# ===========================================================================

FORK_CA_FEED_DAYS = 1        # base value: 2.  None = leave the base alone.
FORK_OPENING = True          # False = leave the base alone.

if FORK_CA_FEED_DAYS is not None:
    _CA_FEED_DAYS = int(FORK_CA_FEED_DAYS)

# step -> (unit index (0 = farmer), expected tape command, expected position,
#          replacement command)
_FORK_OPEN_SCRIPT = {
    2:  (2, ["PASS"], (5, 4), ["WEST"]),
    3:  (2, ["PASS"], (4, 4), ["WEST"]),
    4:  (2, ["PASS"], (3, 4), ["WEST"]),
    5:  (2, ["PASS"], (2, 4), ["PLANT", "WHEAT"]),
    6:  (2, ["PASS"], (2, 4), ["WATER"]),
    29: (3, ["BUILD_PASTURE"], (2, 4), ["WATER"]),
    49: (1, ["PASS"], (5, 4), ["WEST"]),
    50: (1, ["PASS"], (4, 4), ["WEST"]),
    51: (1, ["PASS"], (3, 4), ["WEST"]),
    52: (1, ["PASS"], (2, 4), ["WATER"]),
    ...
    85: (1, ["PASS"], (1, 4), ["EAST"]),
    86: (1, ["PASS"], (2, 4), ["WATER"]),
    87: (1, ["PASS"], (2, 4), ["HARVEST"]),
    88: (1, ["PASS"], (2, 4), ["BUILD_PASTURE"]),
    89: (1, ["PASS"], (2, 4), ["EAST"]),
    90: (1, ["PASS"], (3, 4), ["EAST"]),
    91: (1, ["PASS"], (4, 4), ["DROP"]),
}
_FORK_REPORT = dict(fork_applied=0, fork_skipped=0, fork_seed_bought=0, fork_errors=0)
_FORK_STATE = {}
_FORK_PARENT = agent
del agent


def _fork_overlay(observation, action):
    step = int(observation["step"])
    if step == 0:
        market = [list(o) for o in (action.get("market") or [])]
        if len(market) < MAX_ORDERS and not any(
                len(o) >= 2 and o[:2] == ["BUY_SEED", "WHEAT"] for o in market):
            market.append(["BUY_SEED", "WHEAT", 1])
            _FORK_REPORT["fork_seed_bought"] += 1
            out = dict(action)
            out["market"] = market
            return out
        return action
    entry = _FORK_OPEN_SCRIPT.get(step)
    if entry is None:
        return action
    idx, want, pos, repl = entry
    units = [list(action.get("farmer") or ["PASS"])] + [list(c) for c in (action.get("hands") or [])]
    if idx >= len(units) or units[idx] != want:
        _FORK_REPORT["fork_skipped"] += 1
        return action
    farm = observation["farms"][int(observation["player"])]
    positions = [tuple(farm["farmer"])] + [tuple(h) for h in farm["hands"]]
    if idx >= len(positions) or positions[idx] != pos:
        _FORK_REPORT["fork_skipped"] += 1
        return action
    units[idx] = list(repl)
    _FORK_REPORT["fork_applied"] += 1
    out = dict(action)
...
def agent(observation, configuration=None):
    action = _FORK_PARENT(observation, configuration)
    if not FORK_OPENING:
        return action
    try:
        step = int(observation["step"])
        player = int(observation["player"])
        if step == 0 or _FORK_STATE.get(player, 10 ** 9) >= step:
            if step == 0:
                _FORK_REPORT.update(fork_applied=0, fork_skipped=0,
                                    fork_seed_bought=0, fork_errors=0)
        _FORK_STATE[player] = step
        if not isinstance(action, dict):
            return action
        return _fork_overlay(observation, action)
    except Exception:
        _FORK_REPORT["fork_errors"] += 1
        return action


agent.telemetry = _FORK_REPORT
agent = globals().pop("agent")
