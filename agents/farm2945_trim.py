# agents/farm2945_trim.py - agents/public_farm2945.py VERBATIM (Apache-2.0, upstream
# notices below, unmodified) with ONE switchable overlay appended at the end of this
# file. Same architecture as agents/farm2945_late.py, farm2945_v2.py and
# farm2945_v3.py: the base's code is never edited, the overlay wraps the base's
# `agent` callable and post-processes the action dict it returns, and `agent` is
# re-bound LAST so the framework's "last callable in the module namespace" rule
# (kaggle_environments/agent.py:64) still picks it up.
#
...
#
# THE OVERLAY: TRIM_SEED. Written 2026-09-21 from the dead-spend audit in
# docs/ENDGAME/dead_spend.md, which instrumented the engine (`_do_hire`,
# `_commit_unit`, `_do_buy_land`, `_apply_unit_action`, `_daily_refresh_animals`)
# over 16 self-play episodes (seeds 300-315, both seats = 32 seat-records) and the
# 17 reproducing real farm2945 ladder replays. That audit found the base has almost
# no dead spend: no hire is ever issued at hour 23, no hire is ever refused on cash,
# no PLANT is ever dropped by the engine's seed rule, no market list ever exceeds
# the 10-order cap, no crop is ever planted too late to yield, and every hired hand
# but two $1 ones a season does at least one state-changing op. The single largest
# item that is removable as a pure cap is seed still held, unplanted, at turn 720:
# $137/season in self-play, $67/season on the replays.
#
# Set TRIM_SEED = False and this file emits exactly what the base emits - the flag
# is checked before any other overlay work, so "off" is a single boolean read.
#
# Measured numbers: docs/ENDGAME/dead_spend.md.
...
# ------------------------------------------------------------------ TRIM_SEED
# The one dead-spend item the audit in docs/ENDGAME/dead_spend.md found that can
# be removed as a pure cap: seed bought that turn 720 still holds unplanted.
# Self-play seeds 300-315 both seats: $137/season (max $200). The 17 reproducing
# real replays: $67/season (max $260). Seed cannot be sold and cannot be bought
# back, so every unplanted seed at 720 is cash deleted from the bank.
#
# HOW THE CAP IS APPLIED, AND WHY IT CANNOT SHIFT AN ORDER INDEX.
# `_process_market` walks both players' order lists BY INDEX in lockstep, so
# removing an order from the list moves every later order one index earlier and
# changes when our SELLs price against the opponent's - the exact second-order
# effect that made one seed in the MARGIN_LEVERS seed-trim read +5,446 / -3,226
# on the two seats. `_parse_order` returns None for any order whose quantity is
# <= 0 (kaggriculture.py, `if n <= 0: return None`) and the index loop skips a
# None while `q[:max_orders]` keeps every position, so a fully trimmed order is
# rewritten in place as quantity 0 rather than deleted. Nothing later moves.
#
# THE CAP ITSELF. For crop C, `_TRIM_LAST_PLANT_DAY[C] = 29 - first_yield_day`
# is the last day a planting can still reach its first yield inside the season
# (day index 0-29). Past that day, seed of C can never pay for itself, so the
# cap is 0. ON that day the cap is `_trim_capacity` - the tiles a seed could
# still be planted into today - plus the PLANTs of C already emitted this turn
# (those consume held seed before any later planting does) plus _TRIM_SLACK.
# Nothing is capped before that day, so the whole growing season is untouched.
#
# The rule can never starve a planting the base was going to make, because it
# only ever caps a *purchase* down to the tiles that exist to plant into - it
# never touches a PLANT action, and the engine's all-or-nothing PLANT drop
# (kaggriculture.py:920-931) keys on held seed, which the cap only lets rise to
# what the turn can actually use. `_TRIM_REPORT` counts what it cut so a run can
# be checked against the audit figure instead of against a bank delta.
TRIM_SEED = True          # master flag; False -> byte-identical to the base
_TRIM_SLACK = 2           # extra seed allowed above the capacity count
_TRIM_FIRST_YIELD = {"WHEAT": 2, "CARROT": 2, "TOMATO": 8, "STRAWBERRY": 10, "MELON": 10}
_TRIM_ONGOING = {"WHEAT": False, "CARROT": False, "TOMATO": True,
                 "STRAWBERRY": True, "MELON": False}
_TRIM_LAST_PLANT_DAY = {c: 29 - d for c, d in _TRIM_FIRST_YIELD.items()}
_TRIM_REPORT = {"trim_units": 0, "trim_cash": 0, "trim_orders": 0, "trim_zeroed": 0,
                "trim_turns": 0, "trim_errors": 0}
_TRIM_PARENT = agent


def _trim_capacity(farm, day):
    """Tiles a seed bought right now could still be planted into today.

    Empty tiles, plus the tiles that will BECOME empty inside this same day: a
    WEED cleared by DIG, and a non-`ongoing` crop that is ready to harvest -
    `_apply_unit_action`'s HARVEST branch sets `farm["tiles"][y][x] = None` for
    those (`if not crop_data["ongoing"]`), so the base harvests-then-replants a
    tile within a day and a capacity count that ignored them would starve it.
    Measured: counting only the empty tiles cost a whole CARROT harvest (~$200)
    on 7 of 16 self-play seeds. See docs/ENDGAME/dead_spend.md.
    """
    n = 0
        ...
        for t in row:
            if t is None:
                ...
                if k == "WEED":
                    ...
                    crop = t.get("crop")
                    fy = _TRIM_FIRST_YIELD.get(crop)
                    if (fy is not None and not _TRIM_ONGOING.get(crop)
                            and int(t.get("yield_units", 0) or 0) > 0
                            and day - int(t.get("planted_day", day)) >= fy):
                        n += 1
    return n


def _trim_market(observation, action):
    market = action.get("market")
    if not market:
        return action
    seat = int(observation["player"])
    ...
    for o in market:
        if isinstance(o, list) and len(o) >= 3 and o[0] == "BUY_SEED":
            c = o[1]
            if c in _TRIM_LAST_PLANT_DAY and day >= _TRIM_LAST_PLANT_DAY[c]:
                crops.add(c)
    if not crops:
        return action
    ...
    for a in [action.get("farmer")] + list(action.get("hands") or []):
        if isinstance(a, list) and len(a) >= 2 and a[0] == "PLANT":
            planned[a[1]] = planned.get(a[1], 0) + 1
    capacity = _trim_capacity(farm, day)
    room = {}
    for c in crops:
        if day > _TRIM_LAST_PLANT_DAY[c]:
            cap = 0
        else:
            cap = capacity + _TRIM_SLACK
        room[c] = max(0, cap + planned.get(c, 0) - int(seeds.get(c, 0) or 0))
    out = []
    changed = False
    for o in market:
        if isinstance(o, list) and len(o) >= 3 and o[0] == "BUY_SEED" and o[1] in room:
            c = o[1]
            try:
                q = int(o[2])
            except (TypeError, ValueError):
            ...
            if q > 0:
                ...
                if allow != q:
                    ...
                    o[2] = allow
                    changed = True
                    _TRIM_REPORT["trim_orders"] += 1
                    _TRIM_REPORT["trim_units"] += q - allow
                    _TRIM_REPORT["trim_cash"] += (q - allow) * _TRIM_SEED_PRICE.get(c, 0)
                    if allow == 0:
                        _TRIM_REPORT["trim_zeroed"] += 1
        out.append(o)
    if not changed:
        return action
    _TRIM_REPORT["trim_turns"] += 1
    action = dict(action)
    action["market"] = out
    return action


_TRIM_SEED_PRICE = {"WHEAT": 10, "CARROT": 20, "TOMATO": 50, "STRAWBERRY": 100, "MELON": 80}


def agent(observation, configuration=None):
    action = _TRIM_PARENT(observation, configuration)
    if not TRIM_SEED or not isinstance(action, dict):
        return action
    try:
        return _trim_market(observation, action)
    except Exception:
        _TRIM_REPORT["trim_errors"] += 1
        return action


agent.telemetry = _TRIM_REPORT
agent = globals().pop('agent')
