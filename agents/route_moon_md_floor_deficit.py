# Based on "Kaggriculture Frontier | The Moon Counts Melons" by Kaggle user
# prvsiyan, published on the Kaggriculture competition Code tab under the
# Apache License 2.0 (licence verified on the notebook page, 2026-08-23):
#   https://www.kaggle.com/code/prvsiyan/kaggriculture-frontier-the-moon-counts-melons
#
# Decoded from that notebook's base85+zlib payload and verified byte-for-byte
# against the candidate_sha256 the notebook publishes in its own evidence table
# (ca4f2c0925c38c1cacbb7756ef791f04a45f65660ce4914ad41271ad5798e09f), without
# executing the notebook's code.
#
# CHANGES MADE BY WASHAMBA BOTS (as the Apache 2.0 licence requires be stated):
#
#     _ADAPT_MIN_EVIDENCE            2.0 -> 1.0
#     _PREEMPT_MAX_BATCH              12 -> 24
#     _v17_md_counter lookahead    step+1 -> step+6   (two sites)
#     _min_sell_price_gate         NEW    (a minimum-price gate on all selling)
#     _V88_MIN_POST_SUPPLY_DEFICIT   300 -> 0   (ported from PR #47)
#
# The first three are values; nothing else in the original logic is altered.
# The fourth is new code added by us, described below.
#
# WHY THE THIRD ONE. `_v17_md_counter` is not general market timing - it
# front-runs a PRE-BAKED MODEL OF ONE OPPONENT ARCHETYPE. When the opponent's
# public farm matches a cow-heavy signature (quadrants >= 2, cows >= 4,
# sheep <= 2), it reads a recorded table of what that build is about to sell and
# gets there first. The author left its lookahead at a single step while tuning
# everything else, and the field is overwhelmingly forks of this same lineage,
# so cow-heavy opponents are common.
#
# Measured against this file with only the first two changes, 12 seeds x 2
# seats = 24 matches, against a control returning 4/24 and +0 on the same seeds:
#
#     lead  2    22/24   +358
#     lead  3    18/24   +503
#     lead  4    20/24   +500
#     lead  6    22/24   +598    <- shipped
#     lead  8    22/24   +572
#     lead 10    20/24   +396
#     lead 12    19/24   +251
#
# A clean interior optimum, and it firmed up rather than regressing when the
# sample grew: lead 3 read 11/16 at 8 seeds and 18/24 at 12.
#
# WHAT WE MEASURED AND REJECTED, so nobody re-runs it. All at 8 seeds x 2 seats
# unless noted; "+0" means byte-identical to the control, not merely small:
#
#     _ADAPT_MAX_OPP_HORIZON 6 -> 8, 10          +0   (gate shut AND open)
#     _PREEMPT_MAX_CLONE_DISTANCE 6 -> 10        +0
#     _PREEMPT_START 120 -> 24                   +0
#     _PREEMPT_STOP 680 -> 719                   +0
#     _v17_r5_counter lead 1, 6                  +0   (see below)
#     multi-item preempt instead of single       +0
#     _PREEMPT_MIN_PRICE_RATIO 0.0 -> 0.3/.5/.7  -50 / -320 / -564
#     _PREEMPT_FRACTION 1.0 -> 0.6              -202
#     _DEMAND_ALPHA 0.25 -> 1.0                  -72
#     adaptive confidence margin 0.25 -> 0.1     +13
#
# The price-ratio result is the one worth remembering. Two independent analyses
# agreed that front-running into an already-floored premium market must be
# wasteful, and the gate to prevent it ships disabled. It is a loss at every
# setting and the loss scales with the gate. In a mirror the alternative to
# selling into a crashed market is not selling later at a better price - it is
# selling one step later at the same crashed price, after your clone got there
# first. Being first into a bad market beats being second into it.
#
# KNOWN BLIND SPOT: `_v17_r5_counter` targets a sheep-heavy archetype
# (sheep >= 4 AND cows <= 3). None of this route's own five variants match that,
# so in self-play it never fires and its lead measured +0. That is untestable on
# our harness, NOT proven inert - against real sheep-heavy opponents on the
# ladder it may matter as much as the MD counter does.
#
# See docs/V20_ANATOMY.md and docs/ROUTE_GENERATIONS.md.
...
__version__ = 'Codex-Moon-V92-V91LateTomatoSafe'
...
# --- WASHAMBA BOTS: a minimum-price gate on ALL selling ---------------------
#
# Not the same lever as `_PREEMPT_MIN_PRICE_RATIO`, which is measured and lost
# at every setting (-50 / -320 / -564). That gate only reordered *who dumps
# first* into an already-crashed market, and in a mirror being first into a bad
# market beats being second. This one withholds the volume outright.
#
# Motivation, from three independent directions:
#   * `MARKET_PARAMS`: STRAWBERRY, MILK and WOOL all sit at the $1 floor by
#     ~100 units above the `I0` baseline, so the tail of a large sell earns
#     essentially nothing while still depressing the price for the next day.
#   * top-10 replay data: the leaders bank what we bank on far fewer units -
#     Thomas Tschinkel takes 95,517 on 1,522 units (63 $/unit) against our
#     ~4,544 units at ~20 $/unit.
#   * our own ladder episodes: over the last 40 games of submission 55777536 we
#     bank 87,779 against opponents' 92,592. Our farm is at parity; what we do
#     to the shared market is not.
#
# The gate trims a SELL order to the units whose *marginal* clearing price is
# still at or above `ratio * base_price`, and drops it entirely if none are.
# Two exemptions, both because withheld stock is worth strictly less than a $1
# sale in those states:
#   * late season, when unsold inventory scores nothing (`_MIN_SELL_GATE_STOP`)
#   * a nearly-full shed, where overflow past 100 items is silently discarded
#     (`_MIN_SELL_GATE_ROOM`), which is what `_v17_room_guard` exists to prevent
# MEASURED, head to head against agents/route_moon_md.py. The control arm is
# this same file at ratio 0.0, which returned 2/12 and +0 - byte-identical, so
# the harness is not inventing anything:
#
...
#     0.35           5/12  -285         -
#
# A broad positive plateau from 0.05 to 0.15 rather than a knife edge, decaying
# monotonically past it. But 19/24 is BELOW this repo's shipping bar - the MD
# lead shipped at 22/24 - and every one of these matches is a mirror.
#
# THE MECHANISM IS CONFIRMED AND IT IS SMALL. Seed 0, gated vs control, the gate
# withholds 67 units and touches nothing else: STRAWBERRY 294 -> 273, MILK
# 273 -> 250, WOOL 176 -> 153. The <= $5 band falls 603 -> 529 units and
# $1,506 -> $996. FERTILIZER, WHEAT, MELON, EGG and CARROT are untouched.
#
# WHAT THAT SAME TRACE REFUTED, which matters more than the gate does. The
# motivation for building this was "we sell ~4,544 units at ~$20/unit while the
# leaders take 63 $/unit on a third of the volume." The real mix is $41.5/unit:
#
#     band        units    revenue
#     <= $5         529     $   996
#     $6-25         557     $ 11,570
...
#     > $100        124     $ 21,108
#
# Floor-priced selling is 11% of units and 0.5% of revenue. FERTILIZER alone is
# 2,935 of the 4,598 units at $40.9/u - all of it produced free by the animals,
# none of it bought back - and it is 63% of season revenue. The 10-order-per-turn
# cap binds on 6 turns out of 387, so slots are not a constraint either.
#
# So this gate is a real, consistent, SMALL win. It is NOT the fix for opponent
# suppression, and it should not be recorded as one - the gap where our
# opponents bank ~9,000 more than the leaders' opponents is still unexplained.
#
# THE GATE DOES NOT MAKE US RICHER. IT MAKES THE OPPONENT POORER. Decomposed
# over six episodes, our own bank moves -22 / -740 / +136 (mirror) and
# +93 / -18 / +85 (vs route_v20) - noise around zero, mean about -78. The
# opponent's bank falls -265 / -1,181 / -589 and -132 / -270 / -546 - mean about
# -497, and it falls against v20 too, so this is not a mirror artefact.
#
# That makes it the first lever we have measured that moves the number the
# top-10 replay analysis says we lose on: the leaders' opponents bank ~81,000
# where ours bank ~90,049. It closes maybe 5-10% of that gap.
#
# A METHOD NOTE THAT COST A HARNESS. The first foreign-opponent check compared
# OUR OWN BANK across two episodes against a fixed opponent, and read 14/24 and
# +19 - which looked like a mirror artefact and nearly sank the change. That
# harness is structurally blind here: our bank is flat by construction, so it
# reads ~0 no matter how well a suppression change works. The statistic that
# decides an episode - and that Bradley-Terry counts - is the MARGIN. Note that
# `head_to_head.py` was already measuring margin (it subtracts the two banks in
# the same episode), which is why it and the corrected foreign test agree while
# the own-bank test dissents. Re-measured on margin, with a self-control that
# returned exactly 0/12 and +0:
#
#     vs agents/route_v20.py    12 seeds x 2    22/24   +474   t +4.66
#     vs agents/meta_lead3.py   12 seeds x 2    21/24   +314   t +5.42
#     self-control vs v20        6 seeds x 2     0/12     +0   t  0.00
#
# Three harnesses, two of them decisive by this repo's 22/24 bar, and the
# opponents are three different route lineages rather than clones of us.
#
...
_MIN_SELL_GATE_ROOM = 90      # never withhold when the shed is close to full


def _min_sell_price_gate(obs, action, step):
    if step >= _MIN_SELL_GATE_STOP:
        return action
    if _MIN_SELL_PRICE_RATIO <= 0.0 and not _MIN_SELL_PRICE_BY_ITEM:
        return action
    private = _get(obs, "private", {}) or {}
    shed = dict(_get(private, "shed", {}) or {})
    inventories = [dict(value or {}) for value in list(_get(private, "inventories", []) or [])]
    held = sum(max(0, int(value or 0)) for value in shed.values()) + sum(
        max(0, int(value or 0)) for inventory in inventories for value in inventory.values()
    )
    if held >= _MIN_SELL_GATE_ROOM:
        return action
    ...
    for order in action.get("market") or []:
        ...
        ratio = _MIN_SELL_PRICE_BY_ITEM.get(item, _MIN_SELL_PRICE_RATIO)
        if ratio <= 0.0:
            kept.append(order)
            continue
        floor = _MARKET_PARAMS[item][0] * ratio
        stock = int(_get(inventory, item, 10000) or 0)
        allowed = 0
        while allowed < quantity and _market_price(item, stock + allowed) >= floor:
            allowed += 1
        if allowed > 0:
            kept.append([order[0], order[1], allowed])
    action["market"] = kept
    return action
...
def _v17_md_counter(obs, action, step):
    if _V17_MD_FRACTION <= 0 or not _v17_is_md_family(obs, step) or step + 6 >= len(_V17_MD_MARKETS):
        return action
    targets = {}
    for order in _V17_MD_MARKETS[step + 6]:
...
#
# 300 -> 0, PORTED FROM @BillyMwangiDev's PR #47 (agent/moon-deficit-fix), which
# found and diagnosed this on `route_moon_tuned.py`. `_v65_decision` (the late
# STRAWBERRY->TOMATO replant, evaluated once at step 360) is gated by two
# independent conditions: a $3,200 operating reserve, which is real and
# cost-justified, and this floor, commented only as "generic". Because the code
# always evaluates at the NPV-optimal quantity with no fallback to a smaller
# one, the second gate was silently zeroing every case that cleared the first.
#
# It also explains a result measured on this file on 2026-08-27 and recorded in
# docs/ROUTE_GENERATIONS.md: `_v65_plan_routes`/`_v65_compact_targets` constants
# swept 7/7 BYTE-IDENTICAL and were written off as unreachable dead code. They
# were unreachable *because this gate zeroed the quantity*. That entry measured
# the symptom; this is the cause.
#
# RE-MEASURED ON THIS BASE, because Billy's numbers are against a different
# agent one generation back and a trigger set cannot be assumed to carry over.
# Census of seeds 0-23 reproduces his trigger seeds exactly (10, 13, 18).
#
# Reported PER SEED, not per match: 7 of 11 trigger seeds return a byte-
# identical delta on BOTH seats, so the two seats are one observation twice and
# a match-level denominator overstates n. (Billy's headline 21/24 is closer to
# 10/12 read this way - still a win, and the same caveat applies to the
# match-level 15/21 and 17/21 below.)
#
#     harness                        per-seed   per-match          mean
#     vs agents/route_v20.py            8/11       15/21   +694 (t +1.89)
#     mirror vs this file unmodified    9/11       17/21   +1,248 (t +3.48)
#
# Both harnesses positive and they fail on the SAME seeds - 18 and 47 lose on
# both. Seed 33 flips sign between them (-1,007 / +1,276), which is seed noise,
# not a disagreement about direction.
#
# EXPECTED VALUE IS SMALL AND THE LADDER WILL NOT SEE IT. The code path is
# unreachable on ~75% of seeds by construction, so across all seeds this is
# roughly 0.25 x 1,000 = +250 bank, against a measured ~903 spread between
# byte-identical ladder submissions. Shipped because it is positive on two
# harnesses and risk-free where it does not fire, not because it will move the
# public score.
#
# STILL OPEN, and inherited from PR #47: seeds 18 and 47 are real, reproducible
# losses on both harnesses and on both bases. Billy checked the obvious
# hypothesis (thinner margin -> TOMATO glut) and it does not hold. Not resolved.
_V88_MIN_POST_SUPPLY_DEFICIT = 0
_V65_STATE = {
...
def agent(obs):
    try:
        ...
        action = _min_sell_price_gate(obs, action, step)
