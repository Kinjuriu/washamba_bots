...
#
#   The premium-goods market lead is extended from ONE step to THREE.
#   _future_quantity() looks _LEAD steps ahead instead of 1, and _front_run()
#   sets the repayment step to `step + _LEAD` to match, so total sell volume is
#   unchanged - only the timing moves earlier.
#
#   Rationale: the notebook has 222 copy-and-edits, so a large share of the
#   field runs this identical route and a large share of our matches are
#   mirrors. Our own ladder data shows it: 45 wins in 90 episodes, exactly
#   50.0%. In a mirror the sell queue decides the match, so the lead length is
#   the contested parameter. Measured against the stock one-step route, 4 seeds
#   x 2 seats each:
#
#       lead 2   beats stock  8/8   +1,540
#       lead 3   beats stock  8/8   +1,646     <- shipped
#       lead 4   beats stock  8/8   +1,358
#       lead 6   beats stock  7/8     +670
#       lead 8   beats stock  2/8     -285
#
#   and head to head against the next best, lead 3 beats lead 2 8/8 by +1,905.
#   A genuine interior optimum rather than "more is better".
