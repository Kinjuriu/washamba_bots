# agents/farm2945_late.py - agents/public_farm2945.py VERBATIM (Apache-2.0, notices
# below, unmodified) with a late-game OVERLAY appended at the end of this file.
# The base's code is untouched; the overlay wraps its `agent` callable, calls it,
# and post-processes the returned action dict. `agent` is re-bound LAST so the
# framework's "last callable in the module namespace" rule still picks it up.
#
# Origin of the base: thomastschinkel/the-2945-farm-96-vs-the-top-10-public-bots
# (Apache-2.0, published 2026-09-19, submission 56269928, claimed live 2,944.7).
# Overlay written 2026-09-21 for T5 of docs/superpowers/plans/2026-09-21-endgame.md,
# from docs/ENDGAME/day11_diagnosis.md. Measured numbers: docs/ENDGAME/late_overlay_results.md.
#
# ---------------------------------------------------------------- the two levers
# LEVER 1 (LATE_TOMATO_D11): on day 11 the tape performs its only mass planting
#   after day 8 - 13 PLANT STRAWBERRY on the third quadrant, bought that morning
#   and LOCKED at day 10. STRAWBERRY is `ongoing`, first_yield 10, interval 2, so
#   those tiles deliver on days 21/23/25/27 into a book our own day-5-8 block has
#   already floored (measured quote $1-5 from day 20 on 110055611). TOMATO is also
#   `ongoing`, so every later tape op on those tiles (WATER daily, FERTILIZE d20/d24,
#   HARVEST d21..d29, DIG d27-28) is still valid - zero labour change - and the
#   TOMATO book rises monotonically $60 -> $75 across all 30 days in every episode.
#   Implemented as a 1:1 RENAME, not a prepend: the tape buys seed just in time and
#   on 19 of 19 episodes day 11 is exactly `BUY_SEED STRAWBERRY 13` == `PLANT
#   STRAWBERRY 13`, all 13 on d10-LOCKED tiles, with seeds held == {} at every day
#   start. Renaming both sides preserves the seed ledger exactly, adds no market
#   order (the 10-order cap is untouched) and needs no "one turn earlier" buy: the
#   tape's own buys already precede their plants by >= 1 step, which matters because
#   `interpreter` runs `_apply_unit_action` BEFORE `_process_market`, so a same-turn
#   BUY_SEED settles after the PLANT. Side effect: TOMATO seed is $50 against
#   STRAWBERRY's $100, so the rename also SAVES $650.
# LEVER 2 (LATE_MELON_REPLANT): on day 10 the 12 day-0 melon tiles are harvested;
#   5 become COOP/PASTURE and 7 get `PLANT WHEAT` (verified 7 of 7 on 19 of 19,
#   with day-10 `BUY_SEED WHEAT 7` == 7 plants), starting a ~4-day HARVEST+PLANT
#   WHEAT cycle that runs to day 28. Same 1:1 rename, WHEAT -> MELON. MELON is not
#   `ongoing`, so its HARVEST clears the tile and the tape's wheat cycle resumes;
#   the intermediate HARVESTs no-op on an immature melon (`yield_units>0` passes,
#   then `day-planted_day < first_yield_day` returns without clearing) and the
#   intermediate `PLANT WHEAT` no-op on an occupied tile.
#
# Guards (latched ONCE per player at the first observation of the lever's day, so a
# mid-day bank dip can never rename the buys but not the matching plants - that
# mismatch would make the engine drop the whole turn's plants):
#   L1: live STRAWBERRY tiles >= 20 and bank >= $2,000  (measured at day 11 h0:
#       20 tiles on 18 of 19 episodes, 19 on 109656980; bank $14,586-$18,923)
#   L2: >=1 remembered melon tile and bank >= $2,000    (measured at day 10 h0:
#       $1,867-$4,159 - so L2 is a deliberate no-op on the two poorest openings)
#
# ------------------------------------------------------------- measured results
# `experiments/endgame/ladder_replay.py`'s harness over all 19 reachable 56269928
# episodes (4 verified losses, 3 more the diagnosis found, 10 team-name-matched
# losses, 2 wins). The frozen opponent goes off-path once we diverge, so read
# flips first. Control = the unmodified base: 17 of 19 reproduce the recorded
# bank to $0; the 2 that do not are name-matched (109551974 -212, 109793226 +248).
#
# | config | loss->win | win->loss | mean d bank | mean d margin | margin better |
# |---|---|---|---|---|---|
# | lever 1 (tomato) | 0 | 0 | -278 | -3,067 | 5 of 18 |
# | lever 2 (melon)  | 1 | **1** | -552 | -1,525 | 3 of 17 |
# | both             | 0 | **1** | -1,033 | -4,531 | 5 of 19 |
#
# Neither close loss named in the diagnosis flips: 109727399 -563 -> -1,250 margin
# under L1, 109821873 -1,048 -> -14,289. Lever 2's single flip (109551974
# +11,760) is on one of the two episodes the base does NOT reproduce, and is ~6x
# larger than the lever's own arithmetic allows - a frozen-opponent artefact.
#
# Live-opponent measurements, 8 seeds (300-307) x 2 seats, experiments/tapes/run_agents.py:
#
# | | vs router_yuan_nf_trim | vs the base | self-play mean bank |
# |---|---|---|---|
# | base `public_farm2945.py` | **16-0, +15,982** | - | **89,667** |
# | lever 1 | 9-7, +4,576 | **2-14, -11,138** | 85,294 (**-4,373**) |
#
# Self-play gate seed 0 both builds DONE/DONE, no 3,000 (lever 1: 80,447/75,210;
# flags off: 72,101/72,762, identical to the base).
#
# **BOTH LEVERS SHIP DISABLED.** Nothing here meets the plan's criteria (flip the
# close losses, break 0 wins, mean d >= +1,000, self-play within $300).
#
# The reason lever 1 is a -11,138 head to head is NOT the day-11 trade, and it is
# the one thing worth carrying forward: **the base already has a tomato program,
# and lever 1's own tomatoes are what disqualify it.** `_v219_qualifies` commits
# on day 18 to `BUY_LAND` + `BUY_SEED TOMATO 10` on the fourth quadrant, and one
# of its conditions is `not any(tile['crop'] == 'TOMATO')`. Measured with the
# base's telemetry at seed 300: flags off -> `commitments 1, plant_requests 10,
# confirmed_harvest_units 80, tomato_sale_requests 70`; lever 1 on ->
# `commitments 0`. Ten tomato tiles alive from day 20 to day 29 are traded for
# thirteen that are dead by day 23 (TOMATO max_yield 4 at interval 1, so
# `_daily_refresh_plants` sets `max_lifespan_step` at day 23 for a day-11
# planting). Confirmed on the live ladder too, not just in local self-play: 3 of the 19
# recorded episodes show the base running 10 TOMATO tiles from day 20 to day 28. The
# day-18 qualification is shop-gated (>= 3 of PIZZA_SHOP/FARMERS_MARKET unlocked), which
# is why the diagnosis's 9 sampled episodes all showed zero tomato and concluded - wrongly
# - that no tomato code path exists. See docs/ENDGAME/late_overlay_results.md.
# agents/public_farm2945.py - VERBATIM copy of the public Kaggle notebook
# thomastschinkel/the-2945-farm-96-vs-the-top-10-public-bots (Apache-2.0, published
# 2026-09-19, claimed live 2,944.7 as submission 56269928). Pulled 2026-09-21 by T2 of
# docs/superpowers/plans/2026-09-21-endgame.md. Measured 16-0 of 16 (seeds 300-307,
# both seats) vs agents/router_yuan_nf_trim.py, mean +15,982. Not modified. Upstream
# notices retained below. Local sha256 of the extracted cell recorded in
# docs/ENDGAME/public_inventory.md.
...
# =========================================================================== #
#  LATE-GAME OVERLAY - appended 2026-09-21. Nothing above this line is edited. #
#  See the file header for the mechanism, the guards and the measured numbers. #
# =========================================================================== #

_LATE_BASE_AGENT = globals().pop('agent')

# --- lever switches: flip these to measure one lever at a time --------------
LATE_TOMATO_D11 = False     # lever 1 - measured a loss, ships OFF (see header)
LATE_MELON_REPLANT = False  # lever 2 - measured a loss, ships OFF (see header)

# --- lever constants --------------------------------------------------------
LATE_L1_DAY = 11
LATE_L1_MIN_STRAWBERRY_TILES = 20
LATE_L1_MIN_BANK = 2000
LATE_L2_DAY = 10
LATE_L2_MIN_BANK = 2000

_LATE_STATES = {}
_LATE_REPORT = dict(l1_latched=0, l1_blocked=0, l1_plants=0, l1_buys=0,
                    l1_off_quadrant=0, l1_seed_short=0,
                    l2_latched=0, l2_blocked=0, l2_plants=0, l2_buys=0,
                    l2_off_melon=0, l2_seed_short=0, errors=0)


def _late_get(obj, key, default=None):
    try:
        return obj[key]
    except Exception:
...
def _late_state(player):
    st = _LATE_STATES.get(player)
    if st is None:
        st = {'locked_d10': None, 'melon_tiles': None,
              'l1_latch': None, 'l2_latch': None,
              'l1_abort': False, 'l2_abort': False}
        _LATE_STATES[player] = st
    return st


def late_observe(obs, st):
    """Remember the two tile sets the levers key off, before either fires.

    `melon_tiles` is every tile holding a MELON plant at any point up to day 9
    (the day-0 block); `locked_d10` is every tile still LOCKED on day 10 - the
    freshly bought quadrant is the subset of those that day 11 plants into.
    """
    day = int(_late_get(obs, 'day', 0))
    if day > LATE_L2_DAY:
        return
    tiles = _late_get(obs, 'farms')[int(_late_get(obs, 'player', 0))]['tiles']
    if day < LATE_L2_DAY:
        melon = {(x, y) for y, row in enumerate(tiles) for x, t in enumerate(row)
                 if isinstance(t, dict) and t.get('crop') == 'MELON'}
        if melon:
            st['melon_tiles'] = melon
    else:
        st['locked_d10'] = {(x, y) for y, row in enumerate(tiles)
                            for x, t in enumerate(row) if t == 'LOCKED'}


def _late_unit_positions(farm, action):
    pos = [list(farm['farmer'])] + [list(p) for p in farm['hands']]
    cmds = [action.get('farmer') or ['PASS']] + list(action.get('hands') or [])
    return pos, cmds


def _late_rename(obs, action, st, from_crop, to_crop, tileset, report_prefix):
    """Rename `PLANT from_crop` -> `PLANT to_crop` for every unit standing on a
    tile in `tileset`, and `BUY_SEED from_crop n` -> `BUY_SEED to_crop n` in the
    same turn's market list, in place and 1:1 so list length and order indices
    are unchanged.

    The two sides are renamed together or not at all: the engine drops ALL PLANT
    requests for a crop whose turn demand exceeds held seed, so a half-applied
    rename costs the whole turn's planting.
    """
    player = int(_late_get(obs, 'player', 0))
    farm = _late_get(obs, 'farms')[player]
    seeds = (_late_get(obs, 'private') or {}).get('seeds') or {}
    pos, cmds = _late_unit_positions(farm, action)

    idx = [i for i, c in enumerate(cmds)
           if isinstance(c, list) and len(c) >= 2 and c[0] == 'PLANT' and c[1] == from_crop]
    off = [i for i in idx if i >= len(pos) or tuple(pos[i]) not in tileset]
    if off:
        # Verified 0 of 19 episodes; if it ever happens the invariant this lever
        # rests on is gone, so abort the lever rather than starve that plant of
        # the seed whose purchase we would have renamed away.
        _LATE_REPORT[report_prefix + ('_off_melon' if from_crop == 'WHEAT'
                                      else '_off_quadrant')] += len(off)
        st[report_prefix + '_abort'] = True
        return action

    buys = [o for o in (action.get('market') or [])
            if isinstance(o, list) and len(o) >= 3 and o[0] == 'BUY_SEED' and o[1] == from_crop]
    if not idx and not buys:
        return action
    if idx and int(seeds.get(to_crop, 0)) < len(idx):
        _LATE_REPORT[report_prefix + '_seed_short'] += len(idx)
        return action

    action = copy.deepcopy(action)
    pos2, cmds2 = _late_unit_positions(farm, action)
    for i in idx:
        cmds2[i][1] = to_crop
    action['farmer'] = cmds2[0]
    action['hands'] = cmds2[1:]
    for o in action.get('market') or []:
        if isinstance(o, list) and len(o) >= 3 and o[0] == 'BUY_SEED' and o[1] == from_crop:
            o[1] = to_crop
            _LATE_REPORT[report_prefix + '_buys'] += int(o[2])
    _LATE_REPORT[report_prefix + '_plants'] += len(idx)
    return action


def late_lever1_tomato(obs, action, st):
    """Day 11: the third quadrant is planted TOMATO instead of STRAWBERRY."""
    if int(_late_get(obs, 'day', 0)) != LATE_L1_DAY or st['l1_abort']:
        return action
    if st['l1_latch'] is None:
        player = int(_late_get(obs, 'player', 0))
        farm = _late_get(obs, 'farms')[player]
        live = sum(1 for row in farm['tiles'] for t in row
                   if isinstance(t, dict) and t.get('crop') == 'STRAWBERRY')
        st['l1_latch'] = bool(st['locked_d10']
                              and live >= LATE_L1_MIN_STRAWBERRY_TILES
                              and float(farm.get('money', 0)) >= LATE_L1_MIN_BANK)
        _LATE_REPORT['l1_latched' if st['l1_latch'] else 'l1_blocked'] += 1
    if not st['l1_latch']:
        return action
    return _late_rename(obs, action, st, 'STRAWBERRY', 'TOMATO',
                        st['locked_d10'], 'l1')


def late_lever2_melon(obs, action, st):
    """Day 10: the 7 ex-melon tiles are replanted MELON instead of WHEAT."""
    if int(_late_get(obs, 'day', 0)) != LATE_L2_DAY or st['l2_abort']:
        return action
    if st['l2_latch'] is None:
        player = int(_late_get(obs, 'player', 0))
        farm = _late_get(obs, 'farms')[player]
        st['l2_latch'] = bool(st['melon_tiles']
                              and float(farm.get('money', 0)) >= LATE_L2_MIN_BANK)
        _LATE_REPORT['l2_latched' if st['l2_latch'] else 'l2_blocked'] += 1
    if not st['l2_latch']:
        return action
    return _late_rename(obs, action, st, 'WHEAT', 'MELON',
                        st['melon_tiles'], 'l2')


def agent(observation, configuration=None):
    action = _LATE_BASE_AGENT(observation, configuration)
    try:
        st = _late_state(int(_late_get(observation, 'player', 0)))
        late_observe(observation, st)
        if LATE_MELON_REPLANT:
            action = late_lever2_melon(observation, action, st)
        if LATE_TOMATO_D11:
            action = late_lever1_tomato(observation, action, st)
    except Exception:
        _LATE_REPORT['errors'] += 1
    return action


agent.telemetry = _LATE_REPORT
