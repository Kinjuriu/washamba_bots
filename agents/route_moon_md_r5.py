...
#     _v17_r5_counter lookahead    step+3 -> step+6   (UNMEASURED, see below)
#
# Nothing else. No logic is altered; all three are values.
#
...
#
# THE R5 CHANGE IS AN UNMEASURED BET, AND IS MARKED AS ONE. `_v17_r5_counter`
# is the sibling of the MD counter, aimed at a sheep-heavy archetype
# (sheep >= 4 AND cows <= 3). None of this route's five variants match that,
# and neither does our own main.py (1 cow / 2 sheep at step 400), so it never
# fires on any harness we own: this variant plays BYTE-IDENTICAL episodes to
# the MD-only build across seeds 0-2, one of them an exact tie. The lead is
# moved from 3 to 6 on the prior that the MD counter - structurally the same
# mechanism - peaked at 6 (+598, 22/24) with a plateau through 8. Against a
# real sheep-heavy opponent this could help or hurt; we cannot tell. It is
# shipped on the second active slot precisely because it is free where we can
# measure and only speculative where we cannot.
#
