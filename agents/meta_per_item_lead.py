# Based on "V16-RC5 | High-Score 8C/4S Premium Market Lead" by Kaggle user
# boatlee, published on the Kaggriculture competition Code tab under the
# Apache License 2.0:
#   https://www.kaggle.com/code/boatlee/v16-rc5-high-score-8c-4s-premium-market-lead
#
# That notebook states its route was reconstructed by majority vote across three
# public replays of submission 55440039 (Nikita Lugovoy).
#
# CHANGES MADE BY WASHAMBA BOTS (as the Apache 2.0 licence requires be stated):
#
#   1. The premium-goods market lead becomes PER ITEM instead of one constant.
#      _LEADS replaces _LEAD; _future_quantity() and _front_run() look it up per
#      item. STRAWBERRY and WOOL move to 6; MILK and MELON stay at the stock
#      route's behaviour-equivalent 3.
#
#   2. Repayment is tracked in a per-step ledger (state["ledger"]) rather than a
#      single pending due, because per-item leads mean two pulls can fall due on
#      different steps. Measured to be behaviour-identical to the original
#      single-due version when all leads are equal, so this is machinery for (1),
#      not a change in its own right.
#
#   Total sell volume is unchanged either way - only the timing moves earlier.
#
#   Why per item. The notebook has ~222 copy-and-edits, so much of the ladder
#   runs this identical route and much of our field is mirrors of it. Two agents
#   on the same schedule bank the same money, so the match is decided at the
#   order book. Our own ladder record shows it: in the 1700-1900 rating band
#   where two thirds of our episodes are played, 80% of matches are decided by
#   under 5,000 bank on a ~90,000 economy.
#
#   One lead across four differently-shaped markets can only be right for one of
#   them. Measured one item at a time against the uniform lead of 3, 8 seeds x 2
#   seats, then confirmed at 12 seeds x 2 seats:
#
#       STRAWBERRY  1     1/16   -1,414
#       STRAWBERRY  6    15/16     +983   -> 22/24, +1,016 at 12 seeds
#       STRAWBERRY  10   13/16     +702   -> 20/24,   +725
#       WOOL        6    12/16     +194
#       MILK        6     4/16     -938   (wants less, not more - left at 3)
#       MELON     1/6     5/16       +0   (never front-run at all: stock-bound)
#
#   STRAWBERRY 6 + WOOL 6 together: 23/24 wins, +1,306 mean, +1,351 median,
#   against an exact-copy control that returns 6/24 and +0 on the same seeds.
#
#   MELON is inert here and that is a measurement, not an omission: its emitted
#   sell steps are byte-identical at leads 1, 3 and 6, because the shed holds no
#   spare MELON when a pull is attempted.
...
def _fr_state(obs, step):
    ...
    if step == 0 or step < int(state.get("last_step", -1)):
        state = {"last_step": step, "ledger": {}}
        _FR_STATE[seat] = state
    state["last_step"] = step
    ledger = state.setdefault("ledger", {})
    for stale in [d for d in ledger if d < step]:
...
_LEADS = {"MELON": 3, "MILK": 3, "STRAWBERRY": 6, "WOOL": 6}
_LEAD = 3


def _lead_for(item):
    return _LEADS.get(item, _LEAD)


def _future_quantity(step, item):
    future = step + _lead_for(item)
...
def _repay(action, state, step):
    pending = state.get("ledger", {}).pop(step, None)
    if not pending:
        return action
    due = {str(item): max(0, int(quantity)) for item, quantity in dict(pending).items()}
    action = _copy_action(action)
    market = []
...
def _front_run(action, obs, state, step):
    ...
    if moved:
        for item, quantity in moved.items():
            bucket = state.setdefault("ledger", {}).setdefault(step + _lead_for(item), {})
