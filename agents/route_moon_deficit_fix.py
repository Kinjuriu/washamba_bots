...
#
# Three single constants; no logic is altered.
#
# WHY (first two, shipped 2026-08-23 as `route_moon_tuned.py`).
# `_preempt_shift` infers, per premium item, how many steps ahead the
# opponent is selling and shifts its own sale to one step further out. That
# adaptive path is gated on `_ADAPT_MIN_EVIDENCE`, and at 2.0 the gate is
# almost always shut. Measured against the unmodified notebook, 8 seeds x 2
# seats:
#
#     _ADAPT_MAX_OPP_HORIZON  6 -> 8      3/16   +0   (exactly the control)
#     _ADAPT_MAX_OPP_HORIZON  6 -> 10     3/16   +0   (exactly the control)
#     _PREEMPT_MAX_CLONE_DISTANCE 6 -> 10 3/16   +0   (exactly the control)
#     _PREEMPT_MAX_BATCH     12 -> 24     5/16  +38
#     _ADAPT_MIN_EVIDENCE   2.0 -> 1.0   12/16 +146
#     both of the above                  14/16 +185
#
# Three arms returning *exactly* the control's numbers is the finding: those
# knobs sit downstream of a gate that is shut, so they cannot move anything.
# Open the gate and the batch cap starts to matter - which is why the pair
# beats either alone. Lowering the threshold further (0.5, 0.0) is byte-for-
# byte identical to 1.0, so the gate is a switch rather than a dial.
#
# WHY (third, this file, measured 2026-08-29). `_v65_decision` (a one-time,
# step-360-only decision on converting STRAWBERRY tiles to TOMATO late in the
# season) is blocked by two independent gates: a `$3200` `operating_reserve`
# (a real, cost-justified estimate of execution cost - left alone, clears on
# 24% of seeds) and this separate deficit floor, commented in the source only
# as "generic" - weak justification next to the reserve's real accounting -
# which turned out to be silently killing every real case that cleared the
# first gate, because the code always evaluates at the NPV-optimal quantity
# (always the max, 8) with no fallback to a smaller quantity that might still
# clear a lower floor.
#
# Census, seeds 0-49: the reserve gate clears on 12 of 50 seeds (24%) - 10,
# 13, 18, 32, 33, 34, 39, 40, 41, 42, 45, 47. Measured against the unmodified
# `route_moon_tuned.py`:
#
#     trigger seeds only, 12 x 2 seats = 24 matches    21/24   +1,677
#     unselected range, seeds 0-23, 24 x 2 seats       14/48   +27 (diluted -
#         21 of those 24 seeds are provably untouched, see below)
#
# On a non-trigger seed a loss is structurally impossible, not just unlikely:
# `_v65_decision` only evaluates the deficit check `if quantity > 0`, and the
# reserve gate alone already forces every candidate quantity's NPV negative
# there, so this fix's code path is unreached - which is why the diluted
# 24-seed number looks like an inert control without actually being one.
#
# NOT LOSS-FREE, and that claim would be false if made. Two of the twelve
# trigger seeds are real, reproducible losses: seed 18 (seat0 -5,064 / seat1
# +2,750, net -2,314) and seed 47 (both seats exactly -432, a clean symmetric
# loss). The obvious hypothesis - a thinner safety margin causing a TOMATO
# glut - does not hold: seed 13, the cleanest winner, has the *smallest*
# post-supply deficit of any seed checked (53), smaller than both losers
# (176, 158). The more likely mechanism is the STRAWBERRY-side opportunity
# cost interacting with a *contested* market that a static step-360 snapshot
# can't price against another agent's actual behaviour - not resolved to a
# single lever in this pass. Shipped anyway on the win-rate case: 87.5% on
# the seeds it can ever affect, several six-figure-swing wins, zero risk on
# the other 76% of seeds by construction, clearing the bar the first two
# changes in this file already cleared (19/24, 79%).
#
