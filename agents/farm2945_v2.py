# agents/farm2945_v2.py - agents/public_farm2945.py VERBATIM (Apache-2.0, notices
# below, unmodified) with three INDEPENDENTLY SWITCHABLE overlays appended at the
# end of this file.  Same architecture as agents/farm2945_late.py: the base's code
# is never edited, `agent` is re-bound LAST so the framework's "last callable in
# the module namespace" rule still picks it up, and every lever is a flag at the
# top of the overlay.
#
...
# Overlay written 2026-09-21 for T5d of docs/superpowers/plans/2026-09-21-endgame.md
# from docs/ENDGAME/traces_2026-09-21.md.  Measured numbers: docs/ENDGAME/v2_results.md.
#
# -------------------------------------------------------------- the three levers
# A  V2_V219_OFF - disable the base's own day-18 tomato program.  `_v219_qualifies`
#    is evaluated exactly once, at step 432 (agents/public_farm2945.py:1427), and
#    everything downstream reads `_V219_STATES[player]['committed'/'workers']`,
#    which stay absent when it returns False.  So the whole program is disabled by
#    rebinding that ONE module global - no interception, and byte-identical on the
#    14 of 17 harness episodes where it never qualified anyway.  Traces §1.2:
#    forcing it off is +2,480 bank / +1,867 margin, better on 3 of 3.
#
# B  V2_CARROT_* - the base's V9_CARROT layer maps PLANT/BUY_SEED WHEAT -> CARROT
#    under `V9_CARROT_FIRST_DAY <= day <= V9_CARROT_LAST_DAY` and
#    `wheat_held >= 40` and `prices.CARROT / prices.WHEAT >= 1.8`.  The overlay
#    re-assigns those module globals, which `_v9_carrot` reads at call time, so the
#    base's own gate code does the work.  The swap is seed-backed BY CONSTRUCTION
#    in the base (it decrements a per-turn `carrot_seeds` counter taken from held
#    seed and renames the matching BUY_SEED 1:1), so no widening can make a turn
#    demand more seed than it holds - which matters because the engine drops ALL
#    PLANT requests for a crop whose turn demand exceeds held seed
#    (kaggriculture.py:920-931).  MEASURED on the 17 harness episodes: the ratio is
#    the binding condition and it reaches 1.8 on exactly ONE of them
#    (109694364, days 18-23); it sits at 0.64-1.45 everywhere else.  Per-tile
#    break-even on the tape's fixed 4-day cycle is ratio ~= 1.44 (carrot yields 3
#    and costs $20, wheat yields 4 and costs $10), so anything below ~1.45 plants
#    a worse crop.  Both facts are in docs/ENDGAME/v2_results.md.
#
# C  V2_MELON_STAGGER - the tape plants 12 MELON on day 0 and harvests all 12 on
#    day 10, dumping ~72 units into a book whose ONLY sink is the Town Centre's one
#    unit a day (MELON appears in no SHOPS entry, kaggriculture.py:103), then never
#    sells melon again until the day-29 liquidation.
#      The brief's design - delay `PLANT MELON` on the 7 ex-melon wheat tiles from
#    day 0 to day 4 - IS NOT IMPLEMENTABLE on this tape, and the reason is measured,
#    not argued: `_new_plant` sets `consecutive_unwatered = 1`, so a planting that
#    is not watered the same day is a WEED by morning, and the only day on which
#    the tape gives these tiles a PLANT-then-WATER pair by the same unit is day 0
#    itself (day 1 has no visit at all on 6 of the 7 tiles; days 2-5 have a single
#    WATER and then a MOVE, and rewriting a MOVE desynchronises every later
#    position in that unit's route).  Renaming the day-10 `PLANT WHEAT` slot - the
#    other PLANT+WATER pair - is exactly lever 2 of agents/farm2945_late.py, already
#    measured a loss.
#      What IS implementable is a HARVEST stagger, and it is bounded at two days:
#    a day-0 melon's `max_lifespan_step` is `(0+12+1)*24`, so day 12 is the last
#    day it is alive.  So: suppress the day-10 HARVEST on the 7 tiles, let the
#    day-10 `PLANT WHEAT` no-op on the still-occupied tile (the engine returns
#    before touching the seed, so nothing is consumed and the atomic-PLANT rule is
#    untouched), and convert the day-12 WATER visit into a HARVEST.  Verified on
#    all 17 harness episodes: the 7 tiles are the SAME 7 in every one, and every
#    one of the 7 has a day-12 WATER visit in every one.  Yield is unaffected -
#    the melon is capped at max_yield 6 by day 10 already.  The 42 units are then
#    sold by an appended, paced `SELL MELON` (appended LAST so no existing order's
#    settle index moves).
#      Stated cost: the 5 tiles whose next tape HARVEST is day 13 lose one wheat
#    cycle (the tile is empty from the day-12 rescue until the day-13 replant).
#      Stated arithmetic, up front: the melon book recovers ~1 unit/day, so a
#    two-day delay buys ~2 units of headroom on a `sq` curve - this lever cannot be
#    large, and is included because the brief asked for it measured, not because
#    the arithmetic predicts a win.
...
# =========================================================================== #
#  T5d OVERLAY - appended 2026-09-21. Nothing above this line is edited.       #
#  See the file header for each lever's mechanism, guards and measured numbers.#
# =========================================================================== #

_V2_BASE_AGENT = globals().pop('agent')

# --- lever switches ---------------------------------------------------------
# SHIPPED OFF: no lever met the T5d ship criteria - see docs/ENDGAME/v2_results.md.
# With these values the file is bit-identical to agents/public_farm2945.py, verified
# on all 17 harness episodes ($0 delta, no action divergence).
V2_V219_OFF = False         # lever A: disable the base's day-18 tomato program
V2_MELON_STAGGER = False    # lever C: day-10 melon harvest -> day 12, paced sells

# --- lever B: the base's own V9_CARROT gate constants, re-assigned -----------
# Set to the base's own values = byte-identical. Lower the ratio to widen.
V2_CARROT_RATIO = 1.8
V2_CARROT_FIRST_DAY = 10
V2_CARROT_LAST_DAY = 23

# --- lever C constants ------------------------------------------------------
# The 7 day-0 MELON tiles the tape replants with WHEAT on day 10. Identical on
# all 17 harness episodes and in self-play; the lever aborts if the day-0 melon
# block does not land on exactly these tiles.
V2_MELON_TILES = ((0, 4), (1, 2), (1, 3), (2, 1), (2, 2), (3, 0), (3, 1))
V2_MELON_DUMP_DAY = 10      # the day whose HARVEST is suppressed
V2_MELON_RESCUE_DAY = 12    # last day a day-0 melon is alive (max_lifespan_step)
V2_MELON_FIRST_YIELD_DAY = 10
V2_MELON_SELL_FIRST_DAY = 12
V2_MELON_SELL_PER_TURN = 6

_V2_STATES = {}
_V2_REPORT = dict(v219_suppressed=0, melon_seen=0, melon_abort=0,
                  melon_harvest_suppressed=0, melon_rescued=0,
                  melon_sell_orders=0, melon_sold_units=0, errors=0)


# --- lever A ----------------------------------------------------------------
# `_v219_qualifies` is looked up as a module global at call time by the V219
# wrapper (agents/public_farm2945.py:1427), so rebinding it here is the whole
# disable. Keep the original reachable so the flag can be turned back off.
_V2_V219_QUALIFIES_BASE = _v219_qualifies


def _v219_qualifies(obs, native):
    if V2_V219_OFF:
        _V2_REPORT['v219_suppressed'] += 1
        return False
    return _V2_V219_QUALIFIES_BASE(obs, native)


# --- lever B ----------------------------------------------------------------
V9_CARROT_RATIO = V2_CARROT_RATIO
V9_CARROT_FIRST_DAY = V2_CARROT_FIRST_DAY
V9_CARROT_LAST_DAY = V2_CARROT_LAST_DAY


# --- lever C ----------------------------------------------------------------
def _v2_state(player):
    st = _V2_STATES.get(player)
    if st is None:
        st = {'melon': set(), 'abort': False, 'checked': False, 'step': -1}
        _V2_STATES[player] = st
    return st


def _v2_units(farm, action):
    pos = [tuple(farm['farmer'])] + [tuple(p) for p in farm['hands']]
    cmds = [list(action.get('farmer') or ['PASS'])] + \
           [list(c) for c in (action.get('hands') or [])]
    return pos, cmds


def _v2_is_melon(tile):
    return isinstance(tile, dict) and tile.get('kind') == 'PLANT' \
        and tile.get('crop') == 'MELON'


def _v2_observe(obs, st):
    """Remember which of the 7 known tiles carry the day-0 melon block, and latch
    an abort if the block is not the one this lever was measured on."""
    day = int(obs['step']) // 24
    if day > V2_MELON_DUMP_DAY or st['abort']:
        return
    farm = obs['farms'][int(obs['player'])]
    for (x, y) in V2_MELON_TILES:
        if _v2_is_melon(farm['tiles'][y][x]):
            st['melon'].add((x, y))
    if day == V2_MELON_DUMP_DAY and not st['checked']:
        st['checked'] = True
        _V2_REPORT['melon_seen'] += len(st['melon'])
        if len(st['melon']) != len(V2_MELON_TILES):
            st['abort'] = True
            _V2_REPORT['melon_abort'] += 1


def _v2_melon_stagger(obs, action, st):
    """Suppress the day-10 HARVEST on the 7 tiles; harvest them on day 12 instead
    by converting that day's WATER visit; sell the resulting shed melon at a paced
    rate with an APPENDED order, so no existing order's settle index moves."""
    if st['abort']:
        return action
    day = int(obs['step']) // 24
    farm = obs['farms'][int(obs['player'])]
    changed = False
    pos, cmds = _v2_units(farm, action)
    n = min(len(pos), len(cmds))

    if day == V2_MELON_DUMP_DAY:
        for i in range(n):
            if cmds[i][:1] == ['HARVEST'] and pos[i] in st['melon'] \
                    and _v2_is_melon(farm['tiles'][pos[i][1]][pos[i][0]]):
                cmds[i] = ['PASS']
                changed = True
                _V2_REPORT['melon_harvest_suppressed'] += 1
    elif day == V2_MELON_RESCUE_DAY:
        for i in range(n):
            if cmds[i][:1] != ['WATER'] or pos[i] not in st['melon']:
                continue
            tile = farm['tiles'][pos[i][1]][pos[i][0]]
            if not _v2_is_melon(tile) or int(tile.get('yield_units', 0)) <= 0:
                continue
            if day - int(tile.get('planted_day', day)) < V2_MELON_FIRST_YIELD_DAY:
                continue
            cmds[i] = ['HARVEST']
            changed = True
            _V2_REPORT['melon_rescued'] += 1
    ...
    if day >= V2_MELON_SELL_FIRST_DAY and len(market) < MAX_ORDERS:
        stock = int(projected_shed(result, FarmView(obs)).get('MELON', 0))
        planned = sum(int(o[2]) for o in market
                      if len(o) >= 3 and o[:2] == ['SELL', 'MELON'])
        extra = min(V2_MELON_SELL_PER_TURN, stock - planned)
        if extra > 0 and int(obs['market']['prices'].get('MELON', 0)) > 1:
            market.append(['SELL', 'MELON', extra])
            changed = True
            _V2_REPORT['melon_sell_orders'] += 1
            _V2_REPORT['melon_sold_units'] += extra
    return result if changed else action


def agent(observation, configuration=None):
    action = _V2_BASE_AGENT(observation, configuration)
    if not V2_MELON_STAGGER:
        return action
    try:
        ...
        if st is None or step <= st.get('step', -1):
            _V2_STATES.pop(player, None)
            st = _v2_state(player)
        st['step'] = step
        _v2_observe(observation, st)
        action = _v2_melon_stagger(observation, action, st)
    except Exception:
        _V2_REPORT['errors'] += 1
    return action


agent.telemetry = _V2_REPORT
