# agents/farm2945_rim.py - agents/public_farm2945.py VERBATIM (Apache-2.0, notices
# below, unmodified) with ONE switchable overlay appended at the end of this file.
# Same architecture as agents/farm2945_v3.py / _v2.py / _late.py: the base's body is
# byte-identical from the v9/3 submission banner down; only this
# 8-line file header differs, and tests/test_farm2945_rim.py asserts that byte for
# byte. Overlay = the SELL-ADVANCE RIM ("lead K"). Shipped flags are OFF, at which
# point this file is behaviourally identical to the base (seed-0 gate 72,101/72,762).
# Base provenance and sha256: docs/ENDGAME/public_inventory.md. Results: rim_results.md.
...
# =========================================================================== #
#  RIM OVERLAY - appended 2026-09-21. Nothing above this line is edited.       #
#                                                                             #
#  READ THIS BEFORE ADDING A SELL-ADVANCE LAYER TO THIS BASE.                 #
#                                                                             #
#  The brief for this file asked for `_advance_sells(K)` ported from          #
#  agents/router_yuan_nf_trim.py - "when the tape will SELL product P at step  #
#  s+d (d <= K) and the shed already holds P now, emit that SELL now".        #
#  THIS BASE ALREADY HAS THAT LAYER, in a strictly stronger form, and adding  #
#  a second ledger on top of it would double-advance every lot. The base's    #
#  own sale-lead stack, read out of the source rather than the header:        #
#                                                                             #
#   1. `Chassis._sell_lead` (l.636, "fieldbook _lead_sale") - a ONE-step      #
#      lead: sell what tape[step+1] plans to sell, now, for products other    #
#      than WHEAT/FERTILIZER that the projected shed already holds; the lot   #
#      is netted out of step+1 by `Chassis._apply_suppression` (l.607), so    #
#      quantity is conserved. Gated off at `step % 4 == 0` (the town consumes #
#      every 4 steps, so leading across a consumption tick pays nothing) and  #
#      at shop-unlock boundaries (step % 72 == 0).                            #
#      It is then wrapped twice: `_r36_native_lead` (l.1667) restricts the    #
#      native path to 288 <= step < 696, and `_v9_racepx_lead` (l.3923) gates #
#      each item on being quoted at or above its base price (V9_RACEPX_BASE). #
#                                                                             #
#   2. `_r36_reserve` (l.1683) - the MULTI-step advance, and the real thing   #
#      the brief describes. Window 192 <= step < 696. For every product it    #
#      walks the tape's own future steps `step+1 .. step+horizon`, reserves   #
#      at each due step the quantity that step planned to SELL (minus what is #
#      already owed there), capped by the projected shed, and emits the sum   #
#      as ONE merged `['SELL', item, qty]` now. Each reserved lot is written  #
#      into `players[seat]['sell_state']['r36_debts'][due_step][item]`, and   #
#      `_r36_suppress` (l.1671) nets that debt out of the emitted SELL when   #
#      the episode reaches that step - the same conservation guarantee that   #
#      `_lead_used` gives in router_yuan_nf_trim, but per-quantity instead of #
#      per-(tape, step, index).                                               #
#      Its guards are a superset of the brief's: it BREAKS the scan at a      #
#      future `PICKUP item` or a future `BUY_PRODUCT item` (the WHEAT /       #
#      FERTILIZER round-trip guard, expressed generally), skips any item      #
#      already carrying a SELL or BUY_PRODUCT in THIS turn's market, skips    #
#      items with a pending PICKUP in the courier queue, refuses a price      #
#      under $2, and stops at the 10-order cap.                               #
#                                                                             #
#   3. THE HORIZON IS K, AND IT IS NOT 6. `_r36_reserve` reads                #
#      `_V9_ITEM_HZ[seat]` when set, else `_R37_HORIZONS[seat]`.              #
#      `_R37_HORIZONS` (l.1904-1918) is 2, raised to 3 on a 6-turn rival      #
#      similarity streak, to 4 on a probe match, and HARD-SET to 4 for        #
#      288 <= step < 696. But the v9 RACE layer (l.3853-3869) overwrites      #
#      `_V9_ITEM_HZ[seat]` on EVERY step with                                 #
#          horizon = min(V9_RACE_MAX, max(V9_RACE_DEFAULT, lead + MARGIN))    #
#                  = min(48, max(40, observed_rival_lead + 12))               #
#      for CARROT, TOMATO, STRAWBERRY, MELON, EGG, MILK and WOOL - so the     #
#      shipped base already runs at K = 40..48, an order of magnitude past    #
#      the K = 6 the router lineage ships, and `end` (= step + max(horizon))  #
#      carries WHEAT and FERTILIZER along with it.                            #
#                                                                             #
#   4. Order index. `_v224_sales_first` (l.1501) is already lever 2 of        #
#      docs/MARGIN_LEVERS.md: it bubbles every SELL to the front of the       #
#      market list preserving relative order, and refuses to move one past a  #
#      `BUY_PRODUCT`/`BUY_ANIMAL` of the same item. Applied at step >= 144    #
#      (l.1524), again at step >= 288 (l.1739), and `_r37_reorder_sales` runs #
#      at step >= 288 as well. There is no front-placement left to add.       #
#                                                                             #
#   5. The opening trick is already in the base too. Step 0's market is       #
#      `[['BUY_PRODUCT','WHEAT',20], ['SELL','WHEAT',15]]` - Ahmed Berat      #
#      Ozer's V45 first-turn wheat round trip, the mechanism jaxa623's "2802" #
#      notebook documents and sweeps ("EXP-173 isolate opening market         #
#      sequence" in this file's own header). It is present at n = 20 against  #
#      jaxa623's swept plateau of 25-50 (optimum 30), so the only untested    #
#      thing is the size.                                                     #
#                                                                             #
#  SO THIS OVERLAY IS TWO KNOBS ON THE BASE'S OWN RIM, NOT A SECOND RIM:      #
#    RIM_K          - force the sale-reservation horizon (the base's K).      #
#    RIM_OPENING_N  - resize the step-0 index-0 BUY_PRODUCT WHEAT order.      #
#  Both default to OFF, at which value this file does not execute a single    #
#  statement of its own inside `agent` beyond one falsy check.                #
# =========================================================================== #

_RIM_BASE_AGENT = globals().pop('agent')

# --- lever switches ---------------------------------------------------------
# SHIPPED VALUES: both off. With RIM_K = None and RIM_OPENING_N = 0 this file is
# behaviourally identical to agents/public_farm2945.py.
#
# RIM_K: None leaves the base's adaptive horizon alone (control). An int pins the
# v9 RACE horizon to exactly that many steps for every raced item, by collapsing
# V9_RACE_DEFAULT and V9_RACE_MAX onto it - the RACE layer computes
# min(MAX, max(DEFAULT, lead + MARGIN)), which is identically K when MAX == DEFAULT
# == K. RIM_K = 0 turns the multi-step reservation off entirely (`end <= step` in
# _r36_reserve returns immediately); the base's own one-step _sell_lead survives.
RIM_K = None

# RIM_OPENING_N: 0 leaves step 0 alone. An int rewrites the quantity of the step-0
# index-0 `BUY_PRODUCT WHEAT` order (inserting the pair if the base ever stops
# emitting it, and only while that leaves every base order at index <= 9). The
# paired SELL is deliberately NOT resized: the base buys 20 and sells 15, keeping
# 5 WHEAT, and that 5 is day-0 animal feed.
RIM_OPENING_N = 0

_RIM_REPORT = dict(rim_k=-1, rim_opening=0, rim_open_applied=0, rim_open_refused=0,
                   rim_errors=0)

if RIM_K is not None:
    # Collapse the RACE band onto K. Both names are read by the v9 RACE wrapper at
    # call time (l.3864) as module globals, so rebinding them here is the override.
    V9_RACE_DEFAULT = int(RIM_K)
    V9_RACE_MAX = int(RIM_K)
    _RIM_REPORT["rim_k"] = int(RIM_K)


def _rim_race_horizon(lead):
    """Exactly the expression the v9 RACE wrapper evaluates, exposed for tests."""
    return min(V9_RACE_MAX, max(V9_RACE_DEFAULT, lead + V9_RACE_MARGIN))


def _rim_opening_market(market, n, max_orders=10):
    """Return the step-0 market list with the wheat round trip sized at `n`.

    Two cases, and neither may push a base order past index 9:
      * the base already opens with `BUY_PRODUCT WHEAT q` at index 0 (it does) -
        rewrite q to n in place. Nothing moves, so nothing can be displaced.
      * it does not - insert `BUY_PRODUCT WHEAT n` and `SELL WHEAT n` at 0 and 1,
        and only if the list still fits in `max_orders`.
    Returns (orders, applied) and never mutates the caller's list.
    """
    orders = [list(o) for o in (market or [])]
    if orders and len(orders[0]) >= 3 and orders[0][0] == "BUY_PRODUCT" \
            and orders[0][1] == "WHEAT":
        orders[0][2] = int(n)
        return orders, True
    if len(orders) + 2 > max_orders:
        return orders, False
    return [["BUY_PRODUCT", "WHEAT", int(n)], ["SELL", "WHEAT", int(n)]] + orders, True


def agent(observation, configuration=None):
    action = _RIM_BASE_AGENT(observation, configuration)
    if not RIM_OPENING_N:
        return action
    try:
        if int(_get(observation, "step", -1) or 0) != 0 or not isinstance(action, dict):
            return action
        _RIM_REPORT["rim_opening"] = int(RIM_OPENING_N)
        cap = MAX_ORDERS if isinstance(globals().get("MAX_ORDERS"), int) else 10
        orders, applied = _rim_opening_market(action.get("market") or [], RIM_OPENING_N, cap)
        if not applied:
            _RIM_REPORT["rim_open_refused"] += 1
            return action
        _RIM_REPORT["rim_open_applied"] += 1
        action = dict(action)
        action["market"] = orders
    except Exception:
        _RIM_REPORT["rim_errors"] += 1
    return action


agent.telemetry = _RIM_REPORT
