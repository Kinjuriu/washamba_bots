# agents/farm2945_v3.py - agents/public_farm2945.py VERBATIM (Apache-2.0, notices
# below, unmodified) with ONE switchable overlay appended at the end of this file.
# Same architecture as agents/farm2945_v2.py and agents/farm2945_late.py: the base's
# code is never edited, `agent` is re-bound LAST so the framework's "last callable in
# the module namespace" rule still picks it up, and the lever is a flag at the top of
# the overlay.
#
# Origin of the base: thomastschinkel/the-2945-farm-96-vs-the-top-10-public-bots
# (Apache-2.0, published 2026-09-19, submission 56269928).
# Overlay written 2026-09-21 for T5e. Measured numbers: docs/ENDGAME/v3_results.md.
#
...
# =========================================================================== #
#  T5e OVERLAY - appended 2026-09-21. Nothing above this line is edited.       #
#  Lever G ("goose tail"): force the LAST k animal-purchase ORDERS of the      #
#  player's own route tape to be GOOSE instead of COW/SHEEP.                   #
#                                                                             #
#  Hypothesis (docs/ENDGAME/traces_2026-09-21.md Trace 2): farm2945 runs 17.2  #
#  animals and floors WOOL to 31% and MILK to 36% of base, while EGG is a      #
#  log-shaped book needing 2,000+ units to floor. The marginal pasture animal  #
#  therefore sells into a book this agent has already crushed; a goose sells   #
#  into an open one and costs $300 instead of $400/$500.                       #
#                                                                             #
#  ENGINE FACTS THIS RESTS ON (kaggriculture.py, verified, pinned by tests):   #
#   (a) ANIMALS[...]["structure"]: GOOSE -> COOP, COW/SHEEP -> PASTURE, and    #
#       _apply_unit_action's PLACE branch requires                            #
#       tile["kind"] == ANIMALS[item]["structure"] (l.385-391) - a PLACE GOOSE #
#       on a PASTURE is a SILENT no-op, so the matching BUILD_PASTURE MUST be  #
#       renamed to BUILD_COOP. They are separate ops, so the rename is 1:1.    #
#   (b) Production is collected by an explicit HARVEST unit action on the      #
#       animal tile (l.469-472); yield accrues in _daily_refresh_animals and   #
#       is capped at ANIMALS[...]["max_held"] (GOOSE 4, COW/SHEEP 6), so       #
#       production past the cap is DISCARDED. GOOSE has interval 1 (about two  #
#       units a day with daily care) against SHEEP's 3, so a collection        #
#       cadence tuned for a sheep strands goose production.                    #
#   (c) FEED consumes exactly one WHEAT for every species (l.505-512).         #
#                                                                             #
#  The swap is ONLY ever a 1:1 rename of the tape's own ops - no order is      #
#  added, removed or moved, and no unit action is inserted:                    #
#       BUY_ANIMAL <sp> q  ->  BUY_ANIMAL GOOSE q   (same market slot)         #
#       BUILD_PASTURE      ->  BUILD_COOP           (the tile the PLACE lands  #
#                                                    on, same step and unit)   #
#       PICKUP <sp> q      ->  PICKUP GOOSE q                                  #
#       PLACE  <sp>        ->  PLACE  GOOSE                                    #
#  NO position simulation is used, and that is deliberate: simulating the      #
#  tape's moves for a season drifts from live play (extra hires shift unit     #
#  indices and later layers rewrite commands), and a wrongly paired BUILD is   #
#  exactly the failure that spends $300 on a goose the engine then refuses to  #
#  place.  Instead an order is only swapped when the tape pairs its PLACE with #
#  that SAME unit's own BUILD_PASTURE, on the SAME DAY, with no other          #
#  placement by that unit in between - then the coop's tile is, provably, the  #
#  tile the unit is standing on.  At run time the live position is recorded at #
#  the BUILD and must equal the live position at the PLACE; any mismatch, or   #
#  any emitted command that is not the tape op the plan expected, latches      #
#  `broken` and the lever stops touching the action for the rest of the        #
#  episode.  `v3_placed` counts tiles confirmed to hold a GOOSE afterwards.    #
#                                                                             #
#  `k` therefore counts orders actually SWAPPED, walking backwards from the    #
#  end of the season and skipping orders that do not resolve - so it is "the   #
#  last k swappable purchases", and v3_targets records exactly which.          #
# =========================================================================== #

_V3_BASE_AGENT = globals().pop('agent')

# --- lever switch -----------------------------------------------------------
# SHIPPED VALUE: 0 = off. With 0 this file is bit-identical to
# agents/public_farm2945.py - the overlay returns the base's action object
# unchanged before touching any state.
V3_GOOSE_TAIL_K = 0          # number of trailing non-GOOSE purchase ORDERS to swap

_V3_PASTURE = ("COW", "SHEEP")
_V3_STATE = {}
_V3_REPORT = dict(v3_k=0, v3_targets="", v3_animals=0, v3_plan="", v3_rewrites=0,
                  v3_broken=0, v3_placed=0, v3_errors=0)


def _v3_route_tape(seat):
    native = _IMPL.chassis.players.get(seat)
    if not native or native.get("route") not in _IMPL.chassis.routes:
        return None
...
def _v3_scan(obs):
    """Every animal-program op on this seat's own route tape, keyed by (step, unit).

    NO tile positions: simulating the tape's moves for a whole season drifts from
    live play (extra hires shift unit indices, and later layers rewrite commands),
    and a wrongly paired BUILD is exactly the failure that costs a silent PLACE.
    The (step, unit) keys, by contrast, are the tape's own and are what the base
    emits.  Tiles are resolved from the LIVE unit position at run time instead.

        buys    = [(step, species, qty), ...]      market orders
        builds  = [(step, unit, op), ...]
        pickups = [(step, unit, species, qty), ...]
        places  = [(step, unit, species), ...]
    """
    tape = _v3_route_tape(int(obs["player"]))
    if tape is None:
        return None
    buys, builds, pickups, places = [], [], [], []
    for t in range(len(tape)):
        act = tape[t] if isinstance(tape[t], dict) else {}
        units = [act.get("farmer") or ["PASS"]] + list(act.get("hands") or [])
        for i, cmd in enumerate(units):
            if not cmd:
                continue
            if cmd[0] in ("BUILD_COOP", "BUILD_PASTURE"):
                builds.append((t, i, cmd[0]))
            elif len(cmd) >= 2 and cmd[0] == "PICKUP" and cmd[1] in ANIMAL_COST:
                pickups.append((t, i, cmd[1], int(cmd[2]) if len(cmd) > 2 else 1))
            elif len(cmd) >= 2 and cmd[0] == "PLACE" and cmd[1] in ANIMAL_COST:
                places.append((t, i, cmd[1]))
        for o in (act.get("market") or []):
            if o and o[0] == "BUY_ANIMAL":
                buys.append((t, o[1], int(o[2])))
    return buys, builds, pickups, places


def _v3_resolve(place, builds, pickups, places, claimed):
    """Can this PLACE be turned into a goose by renaming only the tape's own ops?

    Requires the unit's OWN preceding BUILD_PASTURE, on the SAME DAY, with no other
    placement by that unit in between - so the tile that build creates is provably
    the tile this placement lands on, without simulating any position.  Also
    requires the carrying PICKUP to hold nothing but animals we are swapping.
    Returns (build_key, pickup_key) or None.
    """
    tp, up, sp = place
    own_builds = [b for b in builds if b[1] == up and b[0] < tp]
    if not own_builds:
        return None
    tb, ub, op = own_builds[-1]
    if op != "BUILD_PASTURE" or tb // 24 != tp // 24:
        return None
    if any(q[1] == up and tb < q[0] < tp for q in places):
        return None                       # two placements share one build: ambiguous
    carriers = [c for c in pickups if c[1] == up and c[2] == sp and c[0] < tp]
    if not carriers:
        return None
    tk, uk, _sp, qty = carriers[-1]
    if qty != 1 + sum(1 for q in claimed if q[1] == up and q[2] == sp and q[0] > tk):
        return None                       # pickup also carries animals we keep
    return (tb, ub), (tk, uk)


def _v3_plan(obs, k):
    """Swap the last `k` fully resolvable non-GOOSE purchase ORDERS to GOOSE.

    Orders are walked from the END of the season.  A GOOSE order is skipped (it is
    already what we want); a COW/SHEEP order is taken only if EVERY animal in it
    resolves as a pure rename - otherwise it is skipped and the walk continues, so
    `k` counts orders actually swapped, not orders inspected.  Quantities are never
    split, because splitting one order into two would add a market order.
    """
    scanned = _v3_scan(obs)
    if not scanned:
        return None
    buys, builds, pickups, places = scanned
    # purchases and placements of a species pair FIFO
    by_species = {}
    for p in places:
        by_species.setdefault(p[2], []).append(p)
    cursor = {}
    order_places = []
    for t, sp, q in buys:
        got = []
        for _ in range(max(0, q)):
            i = cursor.get(sp, 0)
            cursor[sp] = i + 1
            if i < len(by_species.get(sp, ())):
                got.append(by_species[sp][i])
        order_places.append((t, sp, q, got))
    taken, resolutions, inspected = [], [], []
    for t, sp, q, got in reversed(order_places):
        if len(taken) == k:
            break
        if sp not in _V3_PASTURE:
            continue
        inspected.append("d%d:%s*%d" % (t // 24, sp, q))
        if not got or len(got) != max(0, q):
            continue
        claimed = [p for _t, _sp, _q, ps in taken for p in ps]
        resolved = []
        for place in got:
            siblings = claimed + [p for p in got if p is not place]
            r = _v3_resolve(place, builds, pickups, places, siblings)
            if r is None:
                break
            resolved.append((place, r[0], r[1]))
        if len(resolved) != len(got):
            continue
        taken.append((t, sp, q, got))
        resolutions.extend(resolved)
    if len(taken) < k:
        _V3_REPORT["v3_targets"] = "|".join(inspected) + "|short(%d/%d)" % (len(taken), k)
        return None
    plan = {"cmds": {}, "market": {}, "built": {}}
    for place, bkey, kkey in resolutions:
        tp, up, sp = place
        plan["cmds"][bkey] = ("BUILD",)
        plan["cmds"][kkey] = ("PICKUP", sp)
        plan["cmds"][(tp, up)] = ("PLACE", sp, bkey)
    for t, sp, q, _got in taken:
        plan["market"].setdefault(t, set()).add(sp)
    _V3_REPORT["v3_targets"] = ",".join("d%d:%s*%d" % (t // 24, sp, q) for t, sp, q, _g in taken)
    _V3_REPORT["v3_animals"] = sum(len(g) for _t, _sp, _q, g in taken)
    _V3_REPORT["v3_plan"] = ";".join("%d/%d:%s" % (key[0], key[1], v[0])
                                     for key, v in sorted(plan["cmds"].items()))
    return plan


def _v3_apply(obs, action, st):
    plan = st["plan"]
    step = int(obs["step"])
    seat = int(obs["player"])
    farm = obs["farms"][seat]
    ...
    for i in range(min(len(cmds), len(positions))):
        spec = plan["cmds"].get((step, i))
        if spec is None:
            continue
        cmd = cmds[i]
        if spec[0] == "BUILD":
            if cmd[:1] == ["BUILD_PASTURE"]:
                cmds[i] = ["BUILD_COOP"]
                plan["built"][(step, i)] = positions[i]
                changed = True
                _V3_REPORT["v3_rewrites"] += 1
            else:
                st["broken"] = True
        elif spec[0] == "PICKUP":
            if cmd[:2] == ["PICKUP", spec[1]]:
                cmds[i] = ["PICKUP", "GOOSE"] + list(cmd[2:])
                changed = True
                _V3_REPORT["v3_rewrites"] += 1
            else:
                st["broken"] = True
        elif spec[0] == "PLACE":
            # the coop must be the tile this unit is standing on, or PLACE GOOSE is
            # a silent no-op on a pasture - this is the check that makes the pairing
            # safe without simulating positions
            if cmd[:2] == ["PLACE", spec[1]] and plan["built"].get(spec[2]) == positions[i]:
                cmds[i] = ["PLACE", "GOOSE"] + list(cmd[2:])
                st["sites"][positions[i]] = step // 24
                changed = True
                _V3_REPORT["v3_rewrites"] += 1
            else:
                st["broken"] = True
    market = action.get("market") or []
    if step in plan["market"]:
        wanted = plan["market"][step]
        new_market = []
        for o in market:
            o = list(o)
            if len(o) >= 3 and o[0] == "BUY_ANIMAL" and o[1] in wanted:
                o[1] = "GOOSE"
                changed = True
                _V3_REPORT["v3_rewrites"] += 1
            new_market.append(o)
...
def _v3_confirm(obs, st):
    """Count the swapped tiles that really hold a GOOSE - the engine's PLACE is a
    silent no-op when the structure does not match, so this is the only proof."""
    farm = obs["farms"][int(obs["player"])]
    for pos, day in list(st["sites"].items()):
        if int(obs["step"]) // 24 <= day:
            continue
        tile = farm["tiles"][pos[1]][pos[0]]
        if isinstance(tile, dict) and tile.get("animal") == "GOOSE":
            _V3_REPORT["v3_placed"] += 1
        st["sites"].pop(pos, None)


def agent(observation, configuration=None):
    action = _V3_BASE_AGENT(observation, configuration)
    if not V3_GOOSE_TAIL_K:
        return action
    try:
        seat = int(observation["player"])
        step = int(observation["step"])
        st = _V3_STATE.get(seat)
        if st is None or step <= st.get("step", -1):
            st = _V3_STATE[seat] = {"step": -1, "plan": None, "broken": False,
                                    "ready": False, "sites": {}}
            if step == 0:
                _V3_REPORT.update(v3_k=V3_GOOSE_TAIL_K, v3_targets="", v3_animals=0, v3_plan="",
                                  v3_rewrites=0, v3_broken=0, v3_placed=0, v3_errors=0)
        st["step"] = step
        if not st["ready"]:
            st["ready"] = True
            st["plan"] = _v3_plan(observation, V3_GOOSE_TAIL_K)
        if st["plan"] is None or st["broken"] or not isinstance(action, dict):
            return action
        _v3_confirm(observation, st)
        action = _v3_apply(observation, action, st)
    except Exception:
        _V3_REPORT["v3_errors"] += 1
    return action


agent.telemetry = _V3_REPORT
