...
- **Aggressive selling on the Phase 3 base does not clear the bar, in either of the two forms it was tried — and the second form fails the same way on two independent bases.** Both were run by @future-centaur on `experiment/phase3-aggressive-selling` against a frozen Phase 3 control, on all three harnesses.
  ...
  **The Variant B result is the one worth keeping, because it reproduces.** The same model was ported once before onto a different base (post-PR29 `main`, pre-land, commit `9a6a5c6`) and rejected there for trading a **7,777-unit floor** for a 1,345 mean gain. This port onto Phase 3 does not even get that mean upside, and the variance problem returns. **Two independent ports, two different bases, the same failure mode: the cadence model's mean-vs-floor tradeoff is a structural property of the model, not an artefact of the base it lands on.** Bradley-Terry punishes variance, so this direction is closed unless the floor is addressed directly.
