...
The animal rollout is **SHEEP + COW** (`ACTIVE_ANIMALS`, `MAX_ANIMALS = 2`, Issue #20 Phase B1): build a pasture, buy/pick up/place it, feed and care for it daily, collect fertilizer, harvest wool/milk, and hold back a wheat reserve so selling feed can't starve either. A single animal is a cash constraint, not a market one — a second animal *of the same species* is a large *win* head to head and a **-19,514, 0-of-12 disaster** against a built-in pre-`3b8d36f` (buying it drains the days 3-7 cash trough and both starve), and even post-`3b8d36f` (seed reserve 450) is only **+900, 8/12, t=0.61** — marginal (see below). Forcing genuine species diversity instead of a second SHEEP (`pick_next_animal_species`, least-owned-first) measures **+2,336, 8/12, t=1.89** paired against the 1-sheep baseline — same "not a clean win" shape, shipped anyway per Issue #20's ask to prove the mechanism, not yet a validated strategy win. The **species mix** is chosen by care-bank arithmetic for which species to activate, and by least-owned-first once more than one is active (see below). The logic is data-driven off the engine's `ANIMALS` table, so switching or adding species is a config change.
...
Still unimplemented: `BUY_LAND` (measured against — see dead ends) and shed transfers beyond the fertilizer/wheat/animal paths. `FERTILIZE` and the sheep shipped in V2; a per-turn seed budget (see below) shipped later; `pricing.py` exists as research but is **not** wired into `main.py`.
...
- **More than one animal is a heavy loss — but the recorded *reason* was wrong, and so was one of the measurements.** `MAX_ANIMALS` 2/3/4/6 as geese scored 38,413 / 33,983 / 33,617 / 21,749 against 43,099 for one. Two is not opportunity cost, it is **cash starvation in the days 3-7 trough** (see the second-sheep section above) — and it *wins* head to head, so it is only visible on the built-in harness. Three and four are opportunity cost, as recorded.
  ...
  The **sheep-plus-cow** result (-16,634, 0 of 16) was **unreproducible as written**: `ACTIVE_ANIMALS = ["SHEEP", "COW"]` with `MAX_ANIMALS = 2` built **two sheep and no cow** — `choose_animal_to_build`/`decide_animal_market_actions` each just returned the first `ACTIVE_ANIMALS` entry that cleared a cost/season gate, and SHEEP always cleared it first. **Fixed in Issue #20 Phase B1** (`pick_next_animal_species`: pick whichever eligible species is currently owned fewest of, ties broken by list order) — verified mechanically across 6 seeds, every episode ends the season with exactly one SHEEP *and* one COW placed and alive, not two of either. Economically it's **+2,336 mean, 8/12 seeds, t=1.89** paired against the 1-sheep baseline — better than a second SHEEP alone (+900, 8/12, t=0.61) but the same "not a clean win" shape, for the same reason: buying two animals still dents the days 3-7 cash window even with the $450 seed reserve. **Two independent deep markets is proven achievable, not yet proven a strong win** — revisit once Track A's crew-size-from-tiles work removes more of that early cash pressure.
