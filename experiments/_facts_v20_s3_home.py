...
def leftover_occupant(farm, day, x, y, board_size):
    ...
    S3home: that occupant plants underfoot *above* feed-walk (hour ≤ 21).
...
def choose_unit_action(
    ...
    *walking toward* the shed for one - so two units never both count on
    ...
    # S3home / facts 27+28: underfoot STRAW occupant PLANT sits with
    # WATER/HARVEST, above feed-walk. Wheat-harvesters otherwise leave
    # the empty leftover (step 9 is below 1c) and 6a walk-back is gone.
    # Home / first extra only — not SW (fact 29 K-walk stays below).
    # BUILD and WHEAT fallback stay at step 9. No walk.
    if (
        tile is None
        ...
        q = tile_quadrant(ux, uy, board_size)
        first = first_extra_quadrant(farm)
        on_home_or_first = q == home_quadrant(farm) or (first and q == first)
        if occupant == "STRAWBERRY" and on_home_or_first:
            crop = choose_crop(
                farm,
                state["market_state"],
                private,
                ...
                prefer_crop="STRAWBERRY",
            )
            if crop != "STRAWBERRY" or plant_budget.get("STRAWBERRY", 0) <= 0:
                crop = choose_crop(
                    farm,
                    state["market_state"],
                    private,
            ...
            if crop == "STRAWBERRY" and plant_budget.get("STRAWBERRY", 0) > 0:
                plant_budget["STRAWBERRY"] -= 1
                return act_here(["PLANT", "STRAWBERRY"])
    ...
    # 6a. Removed (S3): walk-to-NE for MELON/STRAW is fact 40/42.
    #     S3home: home/NE STRAW d5–8 plants underfoot *above* feed-walk;
    #     still no walk back onto empty leftover.
    ...
    #    S3home already planted STRAW occupant on home/NE above feed-walk.
    #    BUILD and WHEAT fallback stay here (fact 16; do not lift WHEAT).
