...
def choose_unit_action(
    ...
    *walking toward* the shed for one - so two units never both count on
    ...
    # Fact 20 supplement: through first wool (d0–d5), same-tile CARE on a
    # *fed* animal before walking to a different unfed. Uses `tile`
    # (claimed). Ungated / COLLECT-early stacked the crew and crashed
    # the bank. COLLECT stays at step 3. Unfed underfoot still feed-first.
    if (
        day <= 5
    ...
    # 1c. Fact 20: walk to an unfed animal before local CARE/COLLECT on a
    # *different* tile. Same-tile CARE on a fed animal already fired above.
