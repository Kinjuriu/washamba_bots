...
# Throwaway (FACTS.md facts 15/17 rewritten S4 pace): season ceiling is
# 14. Hold 6 through d10; after second land / carpet, 12 that afternoon;
# 14 from d13; stop. Day-0 beach-head is still 4 via calendar_owned_target
# — not this number. Not dump-8 on carpet afternoon (failed seed 8).
MAX_ANIMALS = 14
...
def calendar_owned_target(day):
    ...
    Rewritten S4 pace (facts 15/17): beach-head 4; +1 cow day 3; +1 cow
    day 5 (4C2S = 6) held through day 10 so the d10 MELON wave and second
    land keep the cash drawer. After land2+carpet: 12 that afternoon
    (d11–12), then 14 from d13, stop. Not dump-8 on d11 (failed seed 8).
    Not the tape 8/10/12/13/14 (EOD d6 is $177). Not the old d11+ shop-mix
    count ramp to 18.
    """
    if day < 3:
        return 4
...
def decide_animal_market_actions(
    ...
    stop (rewritten S4 pace). Post-sell cash (fact 11). Wheat reserve
    keys on owned, not filled (facts 5, 18, 19).
    """
    actions = []
    ...
    if slots > 0 and day == 0:
        ...
        if 3 <= day < 11:
            ...
            # Rewritten S4 pace: after SW window opens, 12 then 14.
            # Mix picks *which*, not a count ramp past the cap.
