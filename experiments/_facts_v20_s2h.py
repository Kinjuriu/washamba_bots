...
CROP_PLANTING_WINDOWS = {
    "MELON": (0, 0),  # fact 27/36 S2h: opening only; never replant
    "STRAWBERRY": (5, 12),
    "CARROT": (21, 25),
...
def leftover_occupant(farm, day, x, y, board_size):
    ...
    First extra (NE): no MELON occupant (S2h — fact 27/36 d0-only;
    underfoot WHEAT from S2 fills until S3 STRAW). Later extras (SW):
    STRAW d7–12, CARROT d21–25. Home NW is not a window occupant here;
    fact 36 MELON-claims home on d0 while opening seed is held, else
    underfoot WHEAT from d0.
    """
    q = tile_quadrant(x, y, board_size)
    if q in later_extra_quadrants_effective(farm, day):
...
def decide_market_actions(
    ...
    # Fact 31 / 30: seed restock — bulk on unlock morning, else dribble cadence.
        ...
        leftover_seed = None
        # S2h: do not restock MELON for NE leftover. Opening bulk is
        # melon_opening_restock_quantity on d0 only.
