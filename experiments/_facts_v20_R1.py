...
def herd_has_feed_urgency(farm):
    """True if any placed animal missed feed yesterday (escape risk)."""
    for row in farm.get("tiles") or []:
        for t in row:
            if isinstance(t, dict) and t.get("animal") and t.get("consecutive_unfed", 0) >= 1:
                return True
    return False
...
def pending_sw_unlock_day(farm, day):
    """Fact 32/33: 2nd BUY_LAND still outstanding on a STRAW-window day."""
    return (
        pending_second_land(farm)
        and STRAW_NEW_LAND_DAY_START <= day <= STRAW_NEW_LAND_DAY_END
...
def fact30_wants_straw_seed(farm, private, day, board_size):
    """Fact 30: dribble STRAW when dry in-window; SW-empty outranks MELON restock.
    ...
    if later_extra_quadrants_effective(farm, day) and has_empty_later_extra_for_occupant(
        ...
        return True
    # Pre-unlock dribble from engine d5 (home NW seed-habit buildup, fact 27).
    return day >= CROP_PLANTING_WINDOWS["STRAWBERRY"][0]
...
def straw_bulk_restock_quantity(
    ...
    if pending_second_land(farm):
        unlocked = farm.get("unlocked_quadrants") or ["NW"]
        floor = max(
            floor,
            LAND_PRICES[len(unlocked) - 1] + MIN_CASH_RESERVE_FOR_LAND_BUYING,
        )
...
def unlock_morning_sell_proceeds(farm, private, market_state):
    ...
    if melon_qty > 0:
        ...
        amount = recommend_sell_quantity(
            product="MELON",
            inventory=inventory.get("MELON", 10000),
            quantity=min(melon_qty, cap),
...
def choose_unit_action(
    ...
    *walking toward* the shed for one - so two units never both count on
    ...
    # R1: unlock carpet before feed-walk when no herd feed urgency.
    sw_carpet = state.get("sw_carpet_day")
    if (
        sw_carpet
        and not herd_has_feed_urgency(farm)
    ...
    if is_shed_adjacent(ux, uy, board_size):
        if animal_in_hand is None and not (
            sw_carpet and plant_budget.get("STRAWBERRY", 0) > 0
        ):
            for animal in ACTIVE_ANIMALS:
    ...
    if animal_in_hand and not (
        state.get("sw_carpet_day") and plant_budget.get("STRAWBERRY", 0) > 0
    ):
    ...
    if (
        ...
        and hour <= LATER_EXTRA_PLANT_LAST_HOUR
        and sw_slots[0] > 0
        and (not sw_carpet or herd_has_feed_urgency(farm))
    ):
        carpet_target = find_empty_later_extra_occupant(
            farm, board_size, ux, uy, day,
            exclude=claimed, seeds=seeds, plant_budget=plant_budget,
...
def nikaangukia_meroni(obs):
    ...
    try:
        ...
        if pending_sw_unlock_day(farm, day) and hour == 0:
            bulk = expected_unlock_bulk_straw(farm, private, state["market_state"])
            if bulk > 0:
