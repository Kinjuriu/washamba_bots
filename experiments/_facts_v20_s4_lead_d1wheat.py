...
# during this inclusive window is STRAW. S3: home and NE leftover is
# STRAW on d5–8 (underfoot only — no crew walk). Fact 28: CARROT occupies
# later-extra leftover in CARROT's window; do not reopen STRAW after d12.
STRAW_NEW_LAND_DAY_START = 7
STRAW_NEW_LAND_DAY_END = 12
STRAW_EARLY_DAY_START = 5
STRAW_EARLY_DAY_END = 8
STRAW_EARLY_RESTOCK_CAP = 8

# Fact 31: bulk STRAW seed on SW unlock morning (v20 tapes ~×23).
STRAW_UNLOCK_BULK_TARGET = 23
...
# Fact 34: MELON first_yield_day=10 — mid-season cash wave. Force-sell
# shed + unit-held (SELL reads shed only; units DROP before market). Cap
# matches tape/v20 chunk sizes (12/24), above ordinary MELON turn cap 15.
MELON_CASH_WAVE_FIRST_DAY = 9
MELON_CASH_WAVE_LAST_DAY = 12
MELON_CASH_WAVE_SELL_CAP = 60

# Fact 33 roles: tape funds first BUY_LAND with same-turn SELL WOOL ×12.
# Ordinary WOOL cap stays 5; pending-first-land turns use this.
FIRST_LAND_WOOL_SELL_CAP = 12

# Fact 36: day-0 MELON opening seed sized to beach-head acreage (v20/tape
# BUY_SEED MELON 12), not MAX_SEED_STOCKPILE=3. Later restock still uses
# the stockpile cadence; only day 0 tops held+field up to this target.
MELON_OPENING_SEED = 12
...
SHED_CAPACITY = 100  # engine shedCapacity; DROP/BUY_PRODUCT need free room
...
# Throwaway (FACTS.md facts 15/16/17 rewritten S4 lead): BUY stops at 14
# (12 after carpet, 14 from d13). BUILD still toward 18 so pens can lead
# (S3ne seed 8: 14 owned / 16 pens). Day-0 beach-head is still 4 via
# calendar_owned_target — not this number. Not dump-8. Not cap-14-on-BUILD.
MAX_ANIMALS = 18
...
# Fact 5/36 supplement: d1 mouths eat leftover from d0, not a d1h0 BUY of
# 4. Contested d1h0 is $24 / wheat $28 / aff=0 — ×4 is a silent no-op.
# Extra 2 at the opening still leaves melon 12 and wheat-seed money for
# ~6 plants (tape 7 ±3). v20 leftovers 3; 4 extra meals steal d0 WHEAT.
D0_WHEAT_OVERNIGHT_LEFTOVER = 2
...
def calendar_owned_target(day):
    """How many animals we are allowed to *buy* by the end of `day`.

    Rewritten S4 lead (facts 15/17): beach-head 4; +1 cow day 3; +1 cow
    day 5 (4C2S = 6) held through day 10. After land2+carpet: 12 that
    afternoon (d11–12), then 14 from d13, stop buying. Pens may still
    lead — see calendar_pen_target. Not dump-8. Not cap-14-on-BUILD.
    """
    if day < 3:
        return 4
    if day < 5:
...
def calendar_pen_target(day):
    """How many pens BUILD may put down by the end of `day`.

    Fact 16: pens may lead owned. Cap-14 on BUILD clipped S3ne seed 8
    from 16 pens to 14 and failed the tripwire. Through d12 this matches
    the buy calendar; from d13 it stays the old 18 ceiling so the
    pen-lead can form.
    """
    if day < 13:
...
def _credit_fert_sell(private, market_state):
    ...
    if not market_state:
        return 0
    n = int((private.get("shed") or {}).get("FERTILIZER", 0) or 0)
    if n <= 0:
        return 0
    inv0 = int((market_state.get("inventory") or {}).get("FERTILIZER", 10000))
    return sum(market_price("FERTILIZER", inv0 + i) for i in range(n))


def _wheat_buy_unit_price(market_state):
    """Post-buy quote for one WHEAT (engine BUY_PRODUCT). Unaffordable
    orders abort at the first unit — do not emit a qty cash cannot fill."""
    if not market_state:
        return 25.0
    inv = int((market_state.get("inventory") or {}).get("WHEAT", 10000))
    params = market_state.get("params")
    return float(market_price("WHEAT", inv - 1, params))


def path_c_animal_cap(farm, private, board_size, day):
    return calendar_pen_target(day)
...
# Fact 33: two purchases only (NE + SW). SE is not part of this shape —
# v20 scale to ~14 animals runs on 3 quads. SW recipe keys on 2nd buy.
...
# Fact 33 day windows per purchase index (n_extra before this buy).
# 0=NE d6–10; 1=SW d11–12.
LAND_PURCHASE_DAY_WINDOWS = (
    (6, 10),   # first BUY_LAND → NE (prefer d6–7; catch-up through d10)
    (11, 12),  # second → SW (STRAW window)
)
...
def find_fertilizer_target(
    farm, board_size, ux, uy, day, exclude=None, prefer_crop=None,
    prefer_quadrant=None,
):
    ...
    Fact 41: `prefer_quadrant` (e.g. "SW") further prefers that quadrant
    among prefer_crop tiles when any such tile wants fertilizer.
    """
    ...
    best_quad_target = None
    best_quad_distance = None
    for y in range(board_size):
        ...
        for x in range(len(row)):
            ...
            if best_distance is None or distance < best_distance:
                ...
                best_target = (x, y)
            crop_ok = (not prefer_crop) or row[x].get("crop") == prefer_crop
            ...
            if (
                prefer_quadrant
                and crop_ok
                and tile_quadrant(x, y, board_size) == prefer_quadrant
            ):
                if best_quad_distance is None or distance < best_quad_distance:
                    best_quad_distance = distance
                    best_quad_target = (x, y)
    if prefer_quadrant and best_quad_target is not None:
        return best_quad_target
...
def count_owned_animals(farm, private, board_size):
    ...
    Do NOT count unfilled pens. Empty pastures are build capacity, not
    owned animals (facts 16/17). Counting them froze the herd at 6 after
    d9 built 8 pens for animals whose BUY_ANIMAL silently failed on cash:
    owned read 8, slots stayed 0, and the two empty pens never filled.
    """
...
def decide_animal_market_actions(
    ...
    5th; d3/d5 +1 cow each (prefix 4C2S held through d10); after d10
    melon + second land, 12 the afternoon carpet clears, 14 from d13,
    stop buying (rewritten S4 lead). Pens may still lead (fact 16).
    Post-sell cash (fact 11). Wheat reserve keys on owned, not filled
    (facts 5, 18, 19).
    """
    actions = []
    ...
    target = calendar_owned_target(day)
    held = species_owned_counts(farm, private, board_size)
    slots = max(0, target - owned)

    unlocked_now = farm.get("unlocked_quadrants") or ["NW"]
    # Fact 33: while NE still pending, do not spend the land drawer on
    # animals. d5 is the hole — the d6–d10 defer left the $400 cow in the
    # shed (owned 6 / placed 5) and d6 post-sell ~$657, short of $1500.
    if len(unlocked_now) <= 1 and 5 <= day <= 10:
        slots = 0

    # Unlock morning / carpet day: land + bulk STRAW + carpet need the
    # crew and cash drawer. Defer scale buys until carpet clears.
    if pending_sw_unlock_day(farm, day) and hour == 0:
        slots = 0
    if board_size is not None and is_sw_carpet_day(farm, day, board_size):
        slots = 0
    ...
    if slots > 0 and day == 0:
        ...
        if 3 <= day < 11:
            ...
            # Rewritten S4 lead: after SW window opens, buy 12 then 14.
            # Mix picks *which*. BUILD still uses calendar_pen_target.
    ...
    #
    # Fact 36: MELON opening (~$960) + full 2× wheat on day 0 leaves ~$12
    # and every later BUY_PRODUCT WHEAT absorbs fert income (v20 opens at
    # ~$25 then recovers via sparse wheat buys + fert sells). Day 0: buy
    # one day of feed plus overnight leftover for d1 (aff at d1h0 is 0).
    # Until first land: restock only when shed+carry is empty (emergency),
    # and only the qty cash can actually fill — sell-reserve still uses 2×.
    unlocked = farm.get("unlocked_quadrants") or ["NW"]
    shed_wheat = private.get("shed", {}).get("WHEAT", 0)
    carried_wheat = sum(
        inv.get("WHEAT", 0) for inv in (private.get("inventories") or []) if isinstance(inv, dict)
    )
    held_wheat = shed_wheat + carried_wheat
    if day == 0:
        # Opening hour only — mid-day top-ups as FEED consumes stock
        # re-drain the MELON-opening cash cushion (was $122 → $3).
        if hour == 0:
            wheat_need = owned + D0_WHEAT_OVERNIGHT_LEFTOVER
        else:
            wheat_need = held_wheat
    elif len(unlocked) <= 1:
        if held_wheat > 0:
            wheat_need = held_wheat  # no restock while any feed remains
        else:
            # Emergency: one buy when empty, not every hour of the day.
            wheat_need = owned if hour == 0 else held_wheat
    else:
        wheat_need = MIN_WHEAT_RESERVE_FOR_FEEDING * owned
    if owned > 0 and obs_money > 0:
        short = wheat_need - held_wheat
        if short > 0:
            px = _wheat_buy_unit_price(market_state)
            aff = int(obs_money // px) if px > 0 else 0
            short = min(int(short), aff)
            if short > 0:
...
def sw_straw_wants_fert(farm, day, board_size):
    """Fact 41: True if any SW STRAWBERRY tile currently wants fertilizer."""
    tiles = farm.get("tiles") or []
    bs = board_size or len(tiles) or 10
    for y, row in enumerate(tiles):
        for x, t in enumerate(row):
            if tile_quadrant(x, y, bs) != "SW":
                continue
            if not (
                isinstance(t, dict)
...
def fact41_post_carpet_fert(farm, day, board_size, state):
    """Fact 41: elevate fert errand after carpet, before DIG/weed.

    Carpet still owns the crew while is_sw_carpet_day. Buy-emit turns keep
    sell_fert_for_buy (facts 9/31) — no elevated fert walk then.
    """
    if state.get("sell_fert_for_buy"):
        return False
    if state.get("sw_carpet_day") or is_sw_carpet_day(farm, day, board_size):
        return False
    return sw_straw_wants_fert(farm, day, board_size)
...
def _is_locked_tile(tile):
    return tile == "LOCKED" or (isinstance(tile, dict) and tile.get("kind") == "LOCKED")


def pending_sw_unlock_day(farm, day):
    """Fact 32/33: 2nd BUY_LAND window day with SW still outstanding."""
    lo, hi = LAND_PURCHASE_DAY_WINDOWS[1]
    return pending_second_land(farm) and lo <= day <= hi


def later_extra_quadrants_effective(farm, day):
    """Later-extra quadrants, including SW when it unlocks this turn (fact 32)."""
    later = later_extra_quadrants(farm)
    if later:
        return later
    if pending_sw_unlock_day(farm, day):
        return {"SW"}
    return set()


def is_later_extra_plantable(farm, day, x, y, board_size, tile):
    """Empty, or LOCKED SW on the pending-unlock turn (market runs before units)."""
    q = tile_quadrant(x, y, board_size)
    if q not in later_extra_quadrants_effective(farm, day):
        return False
    if tile is None:
        return True
    return pending_sw_unlock_day(farm, day) and q == "SW" and _is_locked_tile(tile)
...
def count_first_extra_straw(farm, board_size):
    """Live STRAW plants on the first extra (NE). S3ne bootstrap gate."""
...
def leftover_occupant(farm, day, x, y, board_size):
    """Window occupant for this empty leftover tile, or None.

    Window only — not the wheat fallback. S2 plants WHEAT underfoot when
    this returns None (fact 22/28). Returning WHEAT here would make
    find_empty_later_extra_occupant walk the crew onto SW (fact 40/42).

    S3: home and first extra (NE) leftover is STRAW on d5–8 (fact 27/39).
    Never MELON on NE — that refill is the extra 31 plants. Underfoot
    WHEAT (S2) fills when this returns None. Later extras (SW): STRAW
    d7–12, CARROT d21–25. Do not return WHEAT here (that walks the crew
    onto SW — fact 40/42).
    """
    q = tile_quadrant(x, y, board_size)
    if q in later_extra_quadrants_effective(farm, day):
        ...
        if carrot_window and carrot_window != "__no_gate__":
            ...
            if window_start <= day <= window_end:
                return "CARROT"
        return None
    if STRAW_EARLY_DAY_START <= day <= STRAW_EARLY_DAY_END:
        home = home_quadrant(farm)
        first = first_extra_quadrant(farm)
        if q == home or (first and q == first):
            window = CROP_PLANTING_WINDOWS.get("STRAWBERRY")
            if window and window != "__no_gate__":
                window_start, window_end = window
                if window_start <= day <= window_end:
...
def _melon_window_open(day):
    ...
    if not melon_window or melon_window == "__no_gate__":
        return False
...
def melon_claims_home_leftover(private, day, plant_budget=None):
    ...
    if plant_budget is not None:
        melon_seed = max(melon_seed, plant_budget.get("MELON", 0))
    return melon_seed > 0


def home_quadrant(farm):
    unlocked = farm.get("unlocked_quadrants") or ["NW"]
    return unlocked[0] if unlocked else "NW"


def has_empty_home_leftover(farm, day, board_size):
    """True if any empty tile sits on home (NW) leftover."""
    home = home_quadrant(farm)
    for y, row in enumerate(farm.get("tiles") or []):
        for x, t in enumerate(row):
            if t is not None:
                continue
            if tile_quadrant(x, y, board_size) == home:
                return True
    return False


def count_empty_home_leftover(farm, board_size):
    """How many empty tiles sit on home (NW)."""
    home = home_quadrant(farm)
    n = 0
    for y, row in enumerate(farm.get("tiles") or []):
        for x, t in enumerate(row):
...
def count_empty_wheat_leftover(farm, private, day, board_size):
    """Empty tiles that take WHEAT fallback (S2): no window occupant.

    Home leftover still belongs to MELON while fact 36 claims (seed held
    in the opening window). Not a stock-triggered force-plant.
    """
    n = 0
    home = home_quadrant(farm)
    melon_claims = melon_claims_home_leftover(private, day)
    for y, row in enumerate(farm.get("tiles") or []):
        for x, t in enumerate(row):
            if t is not None:
                continue
            if leftover_occupant(farm, day, x, y, board_size) is not None:
                continue
            if tile_quadrant(x, y, board_size) == home and melon_claims:
                continue
            n += 1
    return n
...
def fact38_wants_wheat_seed(farm, private, day, board_size):
    """S2 / fact 22: restock WHEAT for empty leftover from d0."""
    return count_empty_wheat_leftover(farm, private, day, board_size) > 0


def wheat_product_restock_quantity(farm, private, day, board_size):
    """S2: size BUY_SEED WHEAT to wheat-fallback empties (cap 12).

    Waives MIN_CASH_RESERVE_FOR_SEED_BUYING: that $450 floor is the melon
    repurchase spiral. Do not add this floor to BUY_PRODUCT WHEAT.
    """
    if not fact38_wants_wheat_seed(farm, private, day, board_size):
        return 0
    held = (private.get("seeds") or {}).get("WHEAT", 0)
    empty = count_empty_wheat_leftover(farm, private, day, board_size)
    wanted = max(0, min(empty, 12) - held)
    if wanted <= 0:
        return 0

    crop_info = CROPS.get("WHEAT")
    if not crop_info:
        return 0
    ...
    while quantity < wanted:
        ...
        if seed_cost > money * SEED_SPEND_CAP_FRACTION:
            break
        # Leftover-2 at h0 leaves ~$60. Spending it all on seed drops EOD
        # below MIN_MONEY_TO_HIRE and d1 hires nobody (land miss, bank crash).
        if day == 0 and money - seed_cost < MIN_MONEY_TO_HIRE:
            break
        money -= seed_cost
        quantity += 1
    return quantity
...
def count_empty_straw_early_leftover(farm, day, board_size):
    """Empty home/NE tiles whose S3 occupant is STRAW (d5–8)."""
    if not (STRAW_EARLY_DAY_START <= day <= STRAW_EARLY_DAY_END):
        return 0
    n = 0
    for y, row in enumerate(farm.get("tiles") or []):
        for x, t in enumerate(row):
...
def straw_early_restock_quantity(farm, private, day, board_size):
    """S3 / fact 30: BUY_SEED STRAW for home/NE leftover d5–8.

    Waives MIN_CASH_RESERVE_FOR_SEED_BUYING: d5 EOD is ~$405 on S2, so
    the $450 floor never buys. Same waiver as S2 wheat. Cap
    STRAW_EARLY_RESTOCK_CAP — not the d11 bulk of 23.
    """
    empty = count_empty_straw_early_leftover(farm, day, board_size)
    if empty <= 0:
        return 0
    held = (private.get("seeds") or {}).get("STRAWBERRY", 0)
    wanted = max(0, min(empty, STRAW_EARLY_RESTOCK_CAP) - held)
    if wanted <= 0:
        return 0
...
def has_empty_later_extra_for_occupant(farm, day, board_size, crop=None):
    ...
    for y, row in enumerate(farm.get("tiles") or []):
        for x, t in enumerate(row):
            if not is_later_extra_plantable(farm, day, x, y, board_size, t):
                continue
...
def _straw_window_open(day):
    """True during the STRAWBERRY planting window (fact 30 cadence)."""
    straw_window = CROP_PLANTING_WINDOWS.get("STRAWBERRY")
    if not straw_window or straw_window == "__no_gate__":
        return False
    window_start, window_end = straw_window
    return window_start <= day <= window_end
...
def is_sw_unlock_morning(farm, day, hour):
    """Fact 31: engine day of 2nd BUY_LAND, hour 0."""
    return (
        hour == 0
        and pending_second_land(farm)
        and LAND_BUY_START_DAY <= day <= LAND_BUY_LAST_USEFUL_DAY
    )


def is_sw_carpet_day(farm, day, board_size):
    """Fact 32: unlock-day full crew on SW until carpet lands (~15)."""
    if not (STRAW_NEW_LAND_DAY_START <= day <= STRAW_NEW_LAND_DAY_END):
        return False
    if pending_sw_unlock_day(farm, day):
        return True
    if not later_extra_quadrants(farm):
        return False
    sw_straw = sw_empty = 0
    tiles = farm.get("tiles") or []
    bs = board_size or len(tiles) or 10
    for y, row in enumerate(tiles):
        for x, t in enumerate(row):
            ...
            if t is None:
                ...
                sw_straw += 1
    return sw_straw < 15 and sw_empty > 0


def straw_bulk_restock_quantity(
    farm, private, available_cash=None, target=STRAW_UNLOCK_BULK_TARGET,
    unlock_morning=False, reserve_land=True,
):
    """Fact 31: afford-scaled bulk STRAW buy, reserving cash for pending BUY_LAND.

    `reserve_land=False` on unlock hour 0 when land is deferred to hour 1 so
    the carpet can clear fact 32's ≥15 landing bar (STRAW seed is $100;
    land-floor + 15 seeds does not fit one morning's drawer on seed 0).
    """
    seeds = private.get("seeds", {})
    held = seeds.get("STRAWBERRY", 0)
    wanted = max(0, target - held)
    if wanted <= 0:
        return 0
    ...
    money = available_cash if available_cash is not None else farm.get("money", 0)
    floor = MIN_CASH_RESERVE_FOR_LAND_BUYING if unlock_morning else MIN_CASH_RESERVE_FOR_SEED_BUYING
    if reserve_land and pending_second_land(farm):
        unlocked = farm.get("unlocked_quadrants") or ["NW"]
        land_tail = 0 if unlock_morning else MIN_CASH_RESERVE_FOR_LAND_BUYING
        floor = max(
            floor,
            LAND_PRICES[len(unlocked) - 1] + land_tail,
        )

    spend_cap = 1.0 if unlock_morning else SEED_SPEND_CAP_FRACTION
    quantity = 0
    while quantity < wanted:
        if seed_cost > money:
            break
        if seed_cost > money * spend_cap:
            break
        remaining = money - seed_cost
        if remaining < floor:
            break
        money = remaining
        quantity += 1
...
def unlock_morning_sell_proceeds(farm, private, market_state):
    """Fact 31: MELON + FERT revenue on the unlock-morning sell block."""
    cash = 0
    shed = private.get("shed") or {}
    inventory = market_state.get("inventory") or {}
    melon_qty = shed.get("MELON", 0)
    if melon_qty > 0:
        cap = MAX_SELL_PER_TURN.get("MELON", melon_qty)
        amount = recommend_sell_quantity(
            "MELON",
            inventory.get("MELON", 10000),
            min(melon_qty, cap),
            min_acceptable_price=PRICE_FLOOR,
            max_per_turn=cap,
        )
        if amount > 0:
            cash += _sell_order_proceeds("MELON", amount, market_state)
    fert_qty = shed.get("FERTILIZER", 0)
    if fert_qty > 0:
        cash += _sell_order_proceeds("FERTILIZER", fert_qty, market_state)
    return cash


def expected_unlock_bulk_straw(farm, private, market_state):
    """Bulk STRAW seed the unlock-morning market block will buy (fact 31/32)."""
    unlock_cash = farm.get("money", 0) + unlock_morning_sell_proceeds(
        farm, private, market_state
    )
    return straw_bulk_restock_quantity(
        farm, private, available_cash=unlock_cash, unlock_morning=True,
    )
    ...
    for y, row in enumerate(farm.get("tiles") or []):
        for x, t in enumerate(row):
            ...
            if tile_quadrant(x, y, board_size) not in later:
                continue
            occ = leftover_occupant_on_later_extra(farm, day, x, y, board_size)
            # Window occupants only (STRAW/CARROT). WHEAT fill is underfoot.
            if occ not in ("STRAWBERRY", "CARROT"):
                continue
            # plant_budget credits same-turn BUY_SEED (unlock bulk); obs
            # seeds are still pre-market and would falsely block carpet.
            if plant_budget is not None:
                if plant_budget.get(occ, 0) <= 0:
                    continue
            elif seeds.get(occ, 0) <= 0:
                continue
            distance = abs(x - fx) + abs(y - fy)
            if best_distance is None or distance < best_distance:
                best_distance = distance
...
def find_empty_first_extra_occupant(
    ...
    """Nearest empty first-extra tile (NE) whose occupant we can plant.

    S3ne / fact 29: K-walk after land, d5–8 STRAW window. Occupant only
    (not WHEAT fill — that would walk the crew onto empty wheat).
    """
    first = first_extra_quadrant(farm)
    if not first:
        return None
    if not (STRAW_EARLY_DAY_START <= day <= STRAW_EARLY_DAY_END):
        return None
    # Bootstrap only: once any NE STRAW has landed, stop pulling K off
    # pastures (seed 8 CARE/milk). Remaining NE fill is underfoot + any.
    if count_first_extra_straw(farm, board_size) > 0:
        return None
    exclude = exclude or set()
    seeds = seeds or {}
    best_target = None
...
def decide_land_orders(
    ...
    Fact 33: NE on d6–10, SW on d11–12 only (no SE). Buying SW as soon
    as cash allows on d9 broke the carpet / feed half-stack.
    ...
    if n_extra < len(LAND_PURCHASE_DAY_WINDOWS):
        lo, hi = LAND_PURCHASE_DAY_WINDOWS[n_extra]
        if day < lo or day > hi:
            return []
...
def pending_first_land(farm):
    """NW-only — first BUY_LAND (NE) still outstanding (fact 33)."""
    unlocked = farm.get("unlocked_quadrants") or ["NW"]
    n_extra = len(unlocked) - 1
    return n_extra == 0 and n_extra < MAX_LAND_PURCHASES
...
def list_units(farm):
    """(idx, x, y) for farmer then hands, engine inventory indexing."""
    farmer = farm.get("farmer") or [0, 0]
    spots = [(0, farmer[0], farmer[1])]
    for i, hand in enumerate(farm.get("hands") or []):
        if isinstance(hand, (list, tuple)) and len(hand) == 2:
...
def assign_wool_runner(farm, private, board_size, day):
    """Fact 33 roles: reserve feed labor, then ≤1 closest wool holder.

    Returns unit_idx or None. Pending first land only. Units already on
    unfed with wheat, then nearest wheat-holders, are not the runner.
    """
    if not pending_first_land(farm) or day < LAND_BUY_START_DAY:
        return None
    units = list_units(farm)
    unfed = set(unfed_animal_tiles(farm))
    reserved = set()
    covered = set()
    for idx, ux, uy in units:
        inv = unit_inventory(private, idx)
        if (ux, uy) in unfed and inv.get("WHEAT", 0) > 0:
            reserved.add(idx)
            covered.add((ux, uy))
    remaining = [t for t in unfed if t not in covered]
    holders = [
        (idx, ux, uy)
        for idx, ux, uy in units
        if idx not in reserved and unit_inventory(private, idx).get("WHEAT", 0) > 0
    ]
    for tx, ty in remaining:
        if not holders:
            break
        idx, ux, uy = min(holders, key=lambda u: abs(u[1] - tx) + abs(u[2] - ty))
        reserved.add(idx)
        holders = [h for h in holders if h[0] != idx]
    best = None
    best_d = None
    for idx, ux, uy in units:
        if idx in reserved:
            continue
        if int(unit_inventory(private, idx).get("WOOL", 0) or 0) <= 0:
            continue
        sx, sy = nearest_shed_tile(ux, uy, board_size)
        d = abs(ux - sx) + abs(uy - sy)
        if best is None or d < best_d or (d == best_d and idx < best):
            best = idx
            best_d = d
    return best


def wool_place_qty(farm, private, board_size, runner_idx):
    """How many WOOL the assigned runner will PLACE this turn (shed-adj only)."""
    if runner_idx is None:
        return 0
    for idx, ux, uy in list_units(farm):
        if idx != runner_idx:
            continue
        if not is_shed_adjacent(ux, uy, board_size):
            return 0
        tile = get_tile_at(farm, ux, uy)
        if (
            isinstance(tile, dict)
            ...
            return 0
        n = int(unit_inventory(private, idx).get("WOOL", 0) or 0)
        owned = count_owned_animals(farm, private, board_size)
        reserved = owned * MIN_WHEAT_RESERVE_FOR_FEEDING
        room = melon_drop_room(private, reserved)
        return min(n, room)
    return 0
...
def _pending_unlock_sells(farm, day):
    """Fact 31: include unlock-morning MELON/FERT sells in post-sell estimates."""
    return (
        pending_second_land(farm)
        and LAND_BUY_START_DAY <= day <= LAND_BUY_LAST_USEFUL_DAY
    )
...
def _estimated_post_sell_cash(
    ...
    sell_fert_for_buy=False, board_size=None, wool_place_qty=0,
):
    ...
    for order in decide_market_actions(
        ...
        sell_fert_for_buy=sell_fert_for_buy,
        board_size=board_size,
        sw_unlock_morning=_pending_unlock_sells(farm, day),
        wool_place_qty=wool_place_qty,
    ):
...
def decide_hire_orders(farm, board_size, day, hour, seeds=None):
    ...
    if remaining_season_days(day) < 0:
        return []
    # Fact 35: unlock bulk can leave ~$15 on d10 with ripe MELON on the
    # board. MIN_MONEY_TO_HIRE=20 then hires nobody; the farmer alone cannot
    # clear the wave. First hire is $1 — waive the flat floor when ripe
    # MELON exists, still stop when fib cost exceeds money.
    money = farm.get("money", 0)
    ripe_melon = count_ripe_melon(farm, day)
    waive_hire_floor = in_melon_cash_wave(day) and ripe_melon > 0
    if money < MIN_MONEY_TO_HIRE and not waive_hire_floor:
        return []
    ...
    # Fact 35: when waiving, only hire enough to clear the ripe MELON wave —
    # a full work-driven crew on a $15 drawer burns the wheat safety net
    # (seed 0: 2 escapes after uncapped waive).
    if waive_hire_floor and money < MIN_MONEY_TO_HIRE:
        shortfall = min(shortfall, max(0, ripe_melon))

    hires_today = farm.get("hires_today", 0)
    # Keep a few dollars for BUY_PRODUCT WHEAT when the flat hire floor is
    # waived below MIN_MONEY_TO_HIRE.
    remaining_money = money
    if waive_hire_floor and money < MIN_MONEY_TO_HIRE:
        remaining_money = max(0, money - 5)
    orders = []
...
def in_melon_cash_wave(day):
    """Fact 34/35: engine d9–12 MELON harvest/sell window."""
    return MELON_CASH_WAVE_FIRST_DAY <= day <= MELON_CASH_WAVE_LAST_DAY


def held_product_total(private, product):
    """Sum of `product` across every unit inventory (not shed)."""
    total = 0
    for inv in private.get("inventories") or []:
        if isinstance(inv, dict):
...
def any_unfed_animal(farm):
    """True if any placed animal still needs FEED today."""
    for row in farm.get("tiles") or []:
        for t in row:
            if isinstance(t, dict) and t.get("animal") and not t.get("fed_today"):
                return True
...
def melon_drop_room(private, reserved_wheat):
    """How many MELON units can DROP without crowding out wheat reserve.

    Filling the shed with fruit blocks BUY_PRODUCT WHEAT (capacity check)
    and was the seed-8 late-escape path after an uncapped DROP walk.
    """
    shed = private.get("shed") or {}
    used = sum(int(v or 0) for v in shed.values())
    free = max(0, SHED_CAPACITY - used)
    wheat = int(shed.get("WHEAT", 0) or 0)
    wheat_gap = max(0, int(reserved_wheat or 0) - wheat)
    return max(0, free - wheat_gap)


def count_ripe_melon(farm, day):
    """How many MELON tiles are harvestable right now (fact 35 hire gate)."""
    n = 0
    for row in farm.get("tiles") or []:
        for tile in row:
            if (
...
def count_field_crop(farm, crop):
    """Live plant tiles of `crop` on the farm (any quadrant)."""
    n = 0
    for row in farm.get("tiles") or []:
        for t in row:
...
def melon_opening_restock_quantity(farm, private, day):
    """Fact 36: on day 0, top held+field MELON up to MELON_OPENING_SEED.

    Sizes against cash left after day-0 hire+animal+wheat orders (those
    sit ahead of seeds on the assembled market list — fact 6), so the
    emitted qty matches what the engine can land and plant_budget credit
    does not over-commit (overcommit drops *all* PLANT MELON that turn).

    Does not re-apply MIN_CASH_RESERVE_FOR_SEED_BUYING on the opening
    batch: wheat for the beach-head is already in the same market list,
    and that floor would cap the buy at ~6 (the pre-36 failure mode).
    """
    if day != 0:
        return 0
    held = private.get("seeds", {}).get("MELON", 0)
    have = held + count_field_crop(farm, "MELON")
    if have >= MELON_OPENING_SEED:
        return 0
    wanted = MELON_OPENING_SEED - have

    crop_info = CROPS.get("MELON")
    if not crop_info:
        return 0
    seed_cost = crop_info.get("seed")
    if not seed_cost:
    ...
    money = farm.get("money", 0)
    # Day-0 order list: hires → animals → wheat → seeds. Obs money is
    # pre-spend; subtract beach-head so the batch is engine-realistic.
    if have == 0 and held == 0:
        hire_cost = sum(_hire_cost(i) for i in range(FACTS_DAY0_HIRES))
        animal_cost = (
            ANIMALS.get("COW", {}).get("cost", 400) * 2
            + ANIMALS.get("SHEEP", {}).get("cost", 500) * 2
        )
        wheat_est = (4 + D0_WHEAT_OVERNIGHT_LEFTOVER) * 25
        money = max(0, money - hire_cost - animal_cost - wheat_est)
...
def decide_market_actions(
    ...
    hour=None, sw_unlock_morning=False, wool_place_qty=0,
):
    ...
    if sw_unlock_morning:
        melon_qty = shed.get("MELON", 0)
        if melon_qty > 0 and "MELON" not in already_selling:
            cap = MAX_SELL_PER_TURN.get("MELON", melon_qty)
            amount = recommend_sell_quantity(
                "MELON",
                inventory.get("MELON", 10000),
            ...
            if amount > 0:
                actions.append(["SELL", "MELON", amount])
                already_selling.add("MELON")
    ...
    # Fact 33 roles: same-turn PLACE WOOL then SELL (units then market).
    # Credit only the assigned runner's PLACE, not all held (S3d6 phantom).
    if (
        pending_first_land(farm)
        and day >= LAND_BUY_START_DAY
        and "WOOL" not in already_selling
    ):
        sell_w = int(shed.get("WOOL", 0) or 0) + int(wool_place_qty or 0)
        if sell_w > 0:
            cap = FIRST_LAND_WOOL_SELL_CAP
            amount = recommend_sell_quantity(
                "WOOL",
                inventory.get("WOOL", 10000),
                sell_w,
                min_acceptable_price=SELL_PRICE_THRESHOLDS.get(
                    "WOOL", DEFAULT_SELL_THRESHOLD
                ),
                max_per_turn=cap,
            )
            if amount > 0:
                actions.append(["SELL", "WOOL", amount])
                already_selling.add("WOOL")
    ...
    for product, quantity in shed.items():
        ...
        # Fact 34: same for MELON during the d9–12 cash wave — holding for
        # $180 while the board is ripe is the contested d10 cliff.
        # Contested bind: HARVEST leaves fruit in unit inventory; SELL only
        # reads shed. Count held MELON too so same-turn DROP→SELL works
        # (engine applies units before market).
        force_melon_wave = (
            product == "MELON"
            and in_melon_cash_wave(day)
            and product not in already_selling
        )
        if force_melon_wave:
            sell_quantity = quantity + held_product_total(private, "MELON")
    ...
    # Fact 31 / 30: seed restock — bulk on unlock morning, else dribble cadence.
        ...
        for order in actions:
            if order and order[0] == "SELL":
                product = order[1]
                qty = order[2] if len(order) > 2 else 1
                unlock_cash += _sell_order_proceeds(product, qty, market_state)
        bulk = straw_bulk_restock_quantity(
            farm, private, available_cash=unlock_cash, unlock_morning=True,
            reserve_land=True,
        )
        if bulk > 0:
            actions.append(["BUY_SEED", "STRAWBERRY", bulk])
    elif (early := straw_early_restock_quantity(farm, private, day, board_size)):
        actions.append(["BUY_SEED", "STRAWBERRY", early])
        ...
        leftover_seed = None
        # S3: do not restock MELON for NE leftover. Opening bulk is d0 only.
        if has_empty_later_extra_for_occupant(
            farm, day, board_size, "STRAWBERRY"
        ):
...
def choose_unit_action(
    ...
    melon_drop_budget=None, melon_drop_walkers=None, melon_courier_idx=None,
    wool_runner_idx=None,
):
    """
    ...
    *walking toward* the shed for one - so two units never both count on
    ...
    def try_fert_errand():
        """PICKUP/walk/deliver fertilizer. Fact 41 elevates this before DIG."""
        shed_fertilizer = (private.get("shed") or {}).get("FERTILIZER", 0)
        fert_prefer = None if state.get("sell_fert_for_buy") else "STRAWBERRY"
        # Fact 41: when SW STRAW wants fert, prefer SW quadrant targets.
        fert_quad = "SW" if sw_straw_wants_fert(farm, day, board_size) else None
        if inv.get("FERTILIZER", 0) > 0:
            fertilize_target = find_fertilizer_target(
                farm, board_size, ux, uy, day, exclude=claimed,
                prefer_crop=fert_prefer, prefer_quadrant=fert_quad,
            )
            if fertilize_target:
    ...
    # Fact 33 roles: assigned wool runner PLACE/walk after underfoot FEED
    # and WATER, before HARVEST and feed-walk. PLACE not DROP so wheat
    # stays in hand. Only this unit; everyone else stays on upkeep.
    if (
        wool_runner_idx is not None
        and unit_idx == wool_runner_idx
        and inv.get("WOOL", 0) > 0
        and pending_first_land(farm)
        and day >= LAND_BUY_START_DAY
    ):
        if is_shed_adjacent(ux, uy, board_size):
            owned = count_owned_animals(farm, private, board_size)
            reserved = owned * MIN_WHEAT_RESERVE_FOR_FEEDING
            room = melon_drop_room(private, reserved)
            n = min(int(inv["WOOL"]), room)
            if n > 0:
                return act_here(["PLACE", "WOOL", n])
        moved = walk_to(nearest_shed_tile(ux, uy, board_size), reserve=False)
        if moved:
            return moved

    # 2. Crop HARVEST underfoot stays above feed-walk. The broad
    #    "feed-walk first" ladder lost 2/12 vs `pass` because wheat-carriers
    #    abandoned plants. Animal COLLECT/CARE stay below (step 1c).
    # Fact 32: on carpet day with STRAW budget, leave underfoot MELON so
    # the crew can walk to SW (unlock sells already cashed shed fruit).
    if isinstance(tile, dict) and tile.get("kind") == "PLANT" and is_harvestable(tile, day):
        leave_melon_for_carpet = (
            state.get("sw_carpet_day")
            and plant_budget.get("STRAWBERRY", 0) > 0
            and tile.get("crop") == "MELON"
        )
        if not leave_melon_for_carpet:
    ...
    #     Fact 32: on unlock carpet day, STRAW planting outranks animal pickup.
    sw_carpet = state.get("sw_carpet_day")
    shed_animals = sum(animal_pickup_budget.get(a, 0) for a in ACTIVE_ANIMALS)
    carpet_blocks_pickup = (
        sw_carpet
        and plant_budget.get("STRAWBERRY", 0) > 0
        and shed_animals == 0
    )
    if is_shed_adjacent(ux, uy, board_size):
        if animal_in_hand is None and not carpet_blocks_pickup:
            for animal in ACTIVE_ANIMALS:
    ...
    # 2c2. Facts 16/17: walk to shed for unplaced animals. Pickup only runs
    #     shed-adjacent (2c), so without this walk the d9 sheep stay in the
    #     shed all season while the crew is on SW. On carpet day wait until
    #     STRAW seed is exhausted so fact 32 still gets the full crew.
    if (
        animal_in_hand is None
        and shed_animals > 0
        and not carpet_blocks_pickup
        and not (sw_carpet and plant_budget.get("STRAWBERRY", 0) > 0)
    ):
    ...
    #     Fact 32: carpet day defers carry-walk when STRAW seed is available
    #     and no animals are waiting in the shed.
    if animal_in_hand and not (
        sw_carpet
        and plant_budget.get("STRAWBERRY", 0) > 0
        and shed_animals == 0
    ):
    ...
    # Fact 34: d10 after feed + ripe clear/afternoon, carriers walk-to-shed.
    # DROP amount ≤ shed room minus wheat reserve (shared planned via
    # melon_drop_budget as remaining room). Sell-cap 60 drains fruit.
    # Walker count is not the safety valve — shed room is.
    if in_melon_cash_wave(day) and inv.get("MELON", 0) > 0 and not any_unfed_animal(farm):
        owned = count_owned_animals(farm, private, board_size)
        reserved = owned * MIN_WHEAT_RESERVE_FOR_FEEDING
        room = melon_drop_room(private, reserved)
        if melon_drop_budget is not None:
            room = min(room, melon_drop_budget[0])
        drop_n = min(int(inv["MELON"]), room)
        want_walk = (
            day == 10
        ...
        if drop_n > 0 and is_shed_adjacent(ux, uy, board_size):
            if melon_drop_budget is not None:
                melon_drop_budget[0] = max(0, melon_drop_budget[0] - drop_n)
            return act_here(["DROP", "MELON", drop_n])
        if want_walk:
            moved = walk_to(nearest_shed_tile(ux, uy, board_size), reserve=False)
            if moved:
                return moved
    ...
    # Fact 34 sell half is in decide_market_actions. Do NOT prefer MELON
    # harvest over water/feed here — that starved the herd (seed 0: 3
    # escapes) while only moving sell timing.
    # Fact 32: on unlock carpet day with STRAW budget, do not walk the
    # whole crew onto MELON harvest/water — unlock-morning sells already
    # funded the wave from shed fruit; carpet must land the same day.
    # Underfoot WATER/HARVEST (step 2) still fire for the tile we stand on.
    ...
    # 6a. Removed as an *above-feed* walk (fact 40/42). S3ne reuses K-walk
    #     at 9b (after feed) onto NE empty STRAW once first extra unlocks.
    ...
    #    the season.
    # Fact 41: post-carpet SW STRAW fert outranks non-urgent DIG — fall
    # through to try_fert_errand (still below plant/K-walk at 9/9b).
    if isinstance(tile, dict) and tile.get("kind") == "WEED":
        if not fact41_post_carpet_fert(farm, day, board_size, state):
            return act_here(["DIG"])
        fert_act = try_fert_errand()
        if fert_act:
            return fert_act
        return act_here(["DIG"])
    ...
    #    Fact 32: on unlock carpet day, STRAW planting outranks BUILD so
    #    empty-pen construction cannot steal the last carpet slots
    #    (seed 0 was landing 14 with builders competing).
    if tile is None:
        animal_to_build = None
        if not (
            state.get("sw_carpet_day")
            and plant_budget.get("STRAWBERRY", 0) > 0
        ):
        ...
        prefer = occupant
        if prefer is None:
            if on_home and melon_claims and plant_budget.get("MELON", 0) > 0:
                prefer = "MELON"
            elif plant_budget.get("WHEAT", 0) > 0:
                prefer = "WHEAT"
        if not wait_empty and prefer in ("MELON", "WHEAT"):
            crop = choose_crop(
                farm,
                state["market_state"],
                private,
        ...
        elif not wait_empty and occupant:
            crop = choose_crop(
                farm,
                state["market_state"],
                private,
    ...
    #     S3ne: after first extra unlocks, the same K walks onto NE empty
    #     STRAW until the first NE plant lands (d5–8). After feed (1c).
    #     Not 6a. Not a hire. Not a multi-day occupation.
    if hour <= LATER_EXTRA_PLANT_LAST_HOUR and sw_slots[0] > 0:
        ...
        if leftover_target:
            ...
            if moved:
                return moved
        ne_target = find_empty_first_extra_occupant(
            farm, board_size, ux, uy, day,
            exclude=claimed, seeds=seeds, plant_budget=plant_budget,
        )
        if ne_target:
            sw_slots[0] -= 1
            moved = walk_to(ne_target)
            if moved:
                return moved
    ...
    #     Fact 41 elevates this same errand before DIG when post-carpet.
    fert_act = try_fert_errand()
    if fert_act:
        return fert_act
...
def choose_farmer_action(
    ...
    return choose_unit_action(
        ...
        melon_drop_budget, melon_drop_walkers, melon_courier_idx,
        wool_runner_idx,
    )
...
def nikaangukia_meroni(obs):
    ...
    try:
        ...
        # Fact 33 roles: feed labor first, then ≤1 wool runner. Compute
        # before market so same-turn PLACE credits SELL / post-sell land.
        wool_runner_idx = assign_wool_runner(farm, private, board_size, day)
        place_wool = wool_place_qty(farm, private, board_size, wool_runner_idx)
        ...
        sw_carpet = is_sw_carpet_day(farm, day, board_size)
        state["sw_carpet_day"] = sw_carpet

        # Market pieces needed before unit actions: unlock-day plant_budget
        # must credit the STRAW bulk the market will actually buy (engine
        # runs market before units). expected_unlock_bulk_straw under-counts
        # when MILK/WOOL sells fund the drawer but aren't in the MELON+FERT
        # estimate — seed 0 then carpeted with held seed only (landed 12).
        ...
        land = decide_land_orders(
            ...
            board_size=board_size,
            wool_place_qty=place_wool,
        )
        unlock_preamble = bool(land) and pending_second_land(farm)
        rest = decide_market_actions(
            ...
            board_size=board_size,
            hour=hour,
            sw_unlock_morning=unlock_preamble,
            wool_place_qty=place_wool,
        )
        sells = [o for o in rest if o and o[0] == "SELL"]
        rest_no_sell = [o for o in rest if not (o and o[0] == "SELL")]
        buy_seeds = [o for o in rest_no_sell if o and o[0] == "BUY_SEED"]
        rest_after_seed = [o for o in rest_no_sell if not (o and o[0] == "BUY_SEED")]
        ...
        if pending_sw_unlock_day(farm, day) and hour == 0:
            bulk = sum(
                (o[2] if len(o) > 2 else 1)
                for o in buy_seeds
                if o and o[0] == "BUY_SEED" and o[1] == "STRAWBERRY"
            )
            if bulk <= 0:
                bulk = expected_unlock_bulk_straw(
                    farm, private, state["market_state"]
                )
            if bulk > 0:
                plant_budget["STRAWBERRY"] = plant_budget.get("STRAWBERRY", 0) + bulk
        wheat_budget = {"WHEAT": private.get("shed", {}).get("WHEAT", 0)}
        shed = private.get("shed") or {}
        # Credit this turn's BUY_ANIMAL into the pickup budget: the engine
        # applies market before units, but obs["private"] is still pre-buy.
        animal_pickup_budget = {a: shed.get(a, 0) for a in ACTIVE_ANIMALS}
        for order in animals:
            if order and order[0] == "BUY_ANIMAL" and len(order) > 1:
                qty = order[2] if len(order) > 2 else 1
                animal_pickup_budget[order[1]] = (
                    animal_pickup_budget.get(order[1], 0) + qty
                )
        sw_slots = [min(LATER_EXTRA_CREW_K, max(0, LATER_EXTRA_PLANT_LAST_HOUR + 1 - hour))]
        if is_sw_carpet_day(farm, day, board_size):
            sw_slots = [999]
        # Fact 34: carriers walk; DROP budget = shed room minus wheat reserve.
        owned_now = count_owned_animals(farm, private, board_size)
        melon_drop_budget = [
            melon_drop_room(private, owned_now * MIN_WHEAT_RESERVE_FOR_FEEDING)
        ]
        melon_drop_walkers = [99]
        melon_courier_idx = None
        ...
        farmer_action = choose_farmer_action(
            ...
            animal_pickup_budget, sw_slots, melon_drop_budget, melon_drop_walkers,
            melon_courier_idx, wool_runner_idx,
        )
        hands_actions = [
            choose_unit_action(
        ...
        elif unlock_preamble and land:
            # Fact 31: sell → bulk STRAW → hires → land → animals → rest.
            market = sells + buy_seeds + hires + land + animals + rest_after_seed
        elif buying:
            # Fact 33: any pending BUY_LAND before animals (NE or SW) —
            # animals-first silently burns the land drawer.
            if land:
                market = sells + land + animals + hires + rest_no_sell
            else:
                market = sells + animals + hires + rest_no_sell
        else:
            if land:
