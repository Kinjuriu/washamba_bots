...
def decide_market_actions(
    ...
    # B2: day-8 MELON push when 2nd land is still pending (fund unlock bulk).
    if (
        day == 8
        and pending_second_land(farm)
        and melon_qty := shed.get("MELON", 0)
    ) and melon_qty > 0 and "MELON" not in already_selling:
        cap = MAX_SELL_PER_TURN.get("MELON", melon_qty)
        amount = recommend_sell_quantity(
            product="MELON",
            inventory=inventory.get("MELON", 10000),
    ...
    if (
        ...
        and pending_second_land(farm)
        and melon_qty > 0
        and "MELON" not in already_selling
    ):
        cap = MAX_SELL_PER_TURN.get("MELON", melon_qty)
        amount = recommend_sell_quantity(
            product="MELON",
