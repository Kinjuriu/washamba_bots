...
# Fact 33 / S3d6: tape SELL WOOL 12 then BUY_LAND the same hour. Cap 5
# leaves ~$190 after a $1000 land, below MIN_CASH_RESERVE_FOR_LAND_BUYING.
...
def decide_animal_market_actions(
    ...
    # animal bursts (seed 8 emitted land after animals → NE never unlocked).
    if len(unlocked_now) <= 1 and 6 <= day <= 10:
        slots = 0
...
def pending_first_land(farm):
    """NW-only — first BUY_LAND (NE) still outstanding (fact 33 / S3d6)."""
    unlocked = farm.get("unlocked_quadrants") or ["NW"]
    n_extra = len(unlocked) - 1
...
def decide_market_actions(
    ...
    hour=None, sw_unlock_morning=False,
):
    ...
    for product, quantity in shed.items():
        ...
        # Fact 33 / S3d6: first land is funded by WOOL. Count held so
        # same-turn DROP→SELL clears the $500 floor (cap 12 = tape).
        force_wool_land = (
            product == "WOOL"
        ...
        elif force_wool_land:
            sell_quantity = quantity + held_product_total(private, "WOOL")
        if sell_quantity > 0 and (
            ...
            if force_melon_wave:
                ...
                min_price = SELL_PRICE_THRESHOLDS.get("WOOL", DEFAULT_SELL_THRESHOLD)
            else:
...
def choose_unit_action(
    ...
    *walking toward* the shed for one - so two units never both count on
    ...
    if is_shed_adjacent(ux, uy, board_size):
        # Fact 33 / S3d6: DROP wool at the shed before other shed errands
        # so same-turn SELL can fund BUY_LAND (SELL reads shed only).
        if (
            ...
            drop_n = min(int(inv["WOOL"]), room)
            if drop_n > 0:
                return act_here(["DROP", "WOOL", drop_n])
    ...
    # Fact 33 / S3d6: wool holders walk to shed after underfoot feed/water
    # so DROP can land the same morning. Below WATER (fact 20); above
    # feed-walk so a wool-carrier is not stuck feeding all day.
    if (
        pending_first_land(farm)
        and day >= LAND_BUY_START_DAY
        and inv.get("WOOL", 0) > 0
