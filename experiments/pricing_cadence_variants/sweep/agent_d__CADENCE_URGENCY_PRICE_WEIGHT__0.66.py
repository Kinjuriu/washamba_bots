...
# ---------------------------------------------------------------------
# Selling cadence experiment (experiments/pricing_cadence_experiment_report.md)
# NOT wired into the canonical main.py on `main` - this file is one of
# four generated variants (A/B/C/D) for a head-to-head comparison. See
# the report for what each variant means and why.
# ---------------------------------------------------------------------
...
CADENCE_URGENCY_PRICE_WEIGHT = 0.66
...
def decide_market_actions(
    ...
    # SELLING CADENCE EXPERIMENT: needed by estimate_sell_or_hold_value()
    # and inventory_pressure() below - the same pipeline accounting
    # choose_crop() already relies on, just also read here.
    pipeline = count_pipeline_supply(farm, private)

    for product, quantity in shed.items():
        # The shed also holds animals awaiting placement (e.g. a bought
        # SHEEP before BUILD_PASTURE) - those aren't market products at
        # all. should_sell()'s plain dict .get(product, 0) silently
        # no-opped on them; estimate_sell_or_hold_value() calls
        # market_price() unconditionally, which raises KeyError on an
        # unknown item, so this has to be an explicit guard here instead.
        ...
        # SELLING CADENCE EXPERIMENT, VARIANT D: a large pile relative to
        # what could still sell by season end lowers the accepted price -
        # holding out for a peak is a bad bet when we structurally cannot
        # move the pile in time regardless of price.
        ...
        # SELLING CADENCE EXPERIMENT, VARIANT D: continuous replacement for
        # LIQUIDATION_START_DAY's hard cutoff - urgency ramps smoothly as
        # the season runs out (matches the replay evidence of no day-22
        # cliff), lowering the accepted price and raising the per-turn cap
        # together rather than switching both at once on one fixed day.
