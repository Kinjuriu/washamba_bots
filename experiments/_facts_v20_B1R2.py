...
def decide_animal_market_actions(
    ...
    # R2: defer unlock-morning animal buys to h1 so carpet crew is free.
    if hour == 0 and pending_sw_unlock_day(farm, day):
        slots = 0
...
def straw_bulk_restock_quantity(
    ...
    if pending_second_land(farm):
        unlocked = farm.get("unlocked_quadrants") or ["NW"]
        land_tail = 200 if unlock_morning else MIN_CASH_RESERVE_FOR_LAND_BUYING
        floor = max(
            floor,
