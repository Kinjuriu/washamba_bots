...
# Fact 20 supplement: unit_idx -> (day, x, y) of the tile just FED.
# Next hour that unit CARES if still on the tile. Not walker CARE.
_FEED_STAY = {}
...
def choose_unit_action(
    ...
    *walking toward* the shed for one - so two units never both count on
    ...
    if (
        ...
        feed_claimed.add((ux, uy))
        _FEED_STAY[unit_idx] = (day, ux, uy)
        return act_here(["FEED"])

    # Fact 20 supplement: the unit that just FEED stays one hour for CARE
    # if still on that sheep. Other units do not walk in. Before shed
    # pickup — (4,4) is shed-adjacent and would otherwise PICKUP wheat.
    stay = _FEED_STAY.get(unit_idx)
    if stay:
        sday, sx, sy = stay
        if sday != day or (ux, uy) != (sx, sy):
            _FEED_STAY.pop(unit_idx, None)
        elif (
            isinstance(real_tile, dict)
            and real_tile.get("animal") == "SHEEP"
            and real_tile.get("fed_today")
            and not real_tile.get("cared_today")
        ):
            _FEED_STAY.pop(unit_idx, None)
