"""HORIZON S3ne snapshot: d9–d10 S4 window vs route_v20, seeds 0 and 8.

Fill HORIZON Actual before rewriting S4. The fund check is leftover
after d10 melon $ and second-land spend — not 8-by-d6.

Usage:
    .venv/Scripts/python.exe experiments/_trace_horizon_s3ne.py
    .venv/Scripts/python.exe experiments/_trace_horizon_s3ne.py 0 8
"""
...
LAND2_COST = 2000
LAND_FLOOR = 500
# Rewritten S4: pace toward 14 after d10 melon, stop. Hold-6 today.
TARGET_HERD = 14
...
def run_seed(seed, agent=S3NE):
    ...
    for step in steps:
        for p in (0, 1):
            ...
            if p == 0 and day in (9, 10, 11, 12) and (
                hour in (0, 1, 4, 8, 12, 16, 20, 23) or land_now or buy_a
            ):
                d9_d12_hours.append({
                    "day": day,
                    "hour": hour,
                    **row,
                    ...
                    "hire": sum(1 for o in market if o and o[0] == "HIRE"),
                    "sell_melon": any(
                        o and o[0] == "SELL" and len(o) > 1 and o[1] == "MELON"
                        for o in market
                    ),
                })
...
def print_seed(rep):
    ...
    print("\n" + "#" * 88)
    print(
        f"# HORIZON S3ne  seed={seed}  seat=0  "
    ...
    print("\n--- d6 hourly (EOD leftover; 8-by-d6 still blocked) ---")
    print(
    ...
    print("\n--- 8-by-d6 still blocked (not the S4 rewrite) ---")
    d6e = rep["eod"][0].get(6, {})
    eod_cash = d6e.get("money", 0)
    print(
        f"  EOD d6 owned={d6e.get('owned')} placed={d6e.get('placed')} "
        f"cash={eod_cash:.0f}  first land={_first_land(rep['land'][0])}"
    )
    ...
    print("\n--- d9-d12 selected hours (us) ---")
    print(
        f"  {'d':>2} {'h':>3} {'$':>6} {'own/pl':>7} {'melonF':>6} "
        f"{'heldM':>5} {'shedM':>5} {'shedW':>5} buyA hire land sellM"
    )
    land2_emit = None
    for r in rep["d9_d12_hours"]:
        bu = "+".join(f"{sp}{n}" for sp, n in r["buy_a"]) if r["buy_a"] else "-"
        mark = ""
        if r["land"] and land2_emit is None and r["day"] >= 9:
            land2_emit = r
            mark = " LAND2"
        print(
            ...
            f"{int(r.get('sell_melon', False))}{mark}"
        )

    print("\n--- can Actual fund rewritten S4 (pace to 14 after melon, stop)? ---")
    lands = rep["land"][0]
    land2 = lands[1] if len(lands) >= 2 else None
    d9e = rep["eod"][0].get(9, {})
    d10e = rep["eod"][0].get(10, {})
    d11e = rep["eod"][0].get(11, {})
    d12e = rep["eod"][0].get(12, {})
    owned_d10 = d10e.get("owned", 0)
    need = max(0, TARGET_HERD - owned_d10)
    extra_cows = need * COW_COST
    wheat_need = need * 2
    sm9 = _usd(cash, "us", 9, "SELL_MELON")
    sm10 = _usd(cash, "us", 10, "SELL_MELON")
    sm11 = _usd(cash, "us", 11, "SELL_MELON")
    um9 = _units(cash, "us", 9, "SELL_MELON")
    um10 = _units(cash, "us", 10, "SELL_MELON")
    um11 = _units(cash, "us", 11, "SELL_MELON")
    print(
        f"  EOD d9  ${d9e.get('money', 0):.0f} herd={d9e.get('owned')} "
        f"heldM={d9e.get('held_melon')} pens={d9e.get('pens')} shedW={d9e.get('shed_wheat')}"
    )
    print(
        f"  EOD d10 ${d10e.get('money', 0):.0f} herd={owned_d10} "
        f"heldM={d10e.get('held_melon')} pens={d10e.get('pens')} shedW={d10e.get('shed_wheat')}"
    )
    print(
        f"  EOD d11 ${d11e.get('money', 0):.0f} herd={d11e.get('owned')} "
        f"heldM={d11e.get('held_melon')} pens={d11e.get('pens')} shedW={d11e.get('shed_wheat')}"
    )
    print(
        f"  EOD d12 ${d12e.get('money', 0):.0f} herd={d12e.get('owned')}"
    )
    print(
        f"  SELL_MELON d9 ${sm9:.0f}/{um9:.0f}u  d10 ${sm10:.0f}/{um10:.0f}u  "
        f"d11 ${sm11:.0f}/{um11:.0f}u"
    )
    print(f"  second BUY_LAND us={land2}  cash at emit="
          f"{land2_emit['money'] if land2_emit else None}")
    print(
        f"  to hit {TARGET_HERD} from d10 herd {owned_d10}: need {need} head "
        f"(~${extra_cows} cows / ${need * SHEEP_COST} sheep) + wheat {wheat_need}"
    )
    print(
        f"  second land ${LAND2_COST} + floor ${LAND_FLOOR} = "
        f"${LAND2_COST + LAND_FLOOR} reserved; extra head must not steal that."
    )
    d10_cash = d10e.get("money", 0)
    leftover_after_land_reserve = d10_cash - LAND2_COST - LAND_FLOOR
    print(
        f"  EOD d10 ${d10_cash:.0f} - land2 ${LAND2_COST} - floor ${LAND_FLOOR} "
        f"= ${leftover_after_land_reserve:.0f} leftover for extra head"
    )
    print(
        f"  leftover vs 1 cow ${COW_COST}: "
        f"{'FITS +1' if leftover_after_land_reserve >= COW_COST else 'DOES NOT FIT +1'}"
    )
    print(
        f"  leftover vs {need} cows ${extra_cows}: "
        f"{'FITS all-14 same morning' if leftover_after_land_reserve >= extra_cows else 'DOES NOT FIT all-14 from d10 leftover'}"
    )
    if land2_emit is not None:
        leftover_at_land2 = land2_emit["money"] - LAND2_COST
        print(
            f"  cash at land2 emit ${land2_emit['money']:.0f} - ${LAND2_COST} = "
            f"${leftover_at_land2:.0f}; after floor "
            f"${leftover_at_land2 - LAND_FLOOR:.0f} vs 1 cow ${COW_COST} "
            f"{'FITS +1 after land2' if leftover_at_land2 - LAND_FLOOR >= COW_COST else 'DOES NOT FIT +1 after land2'}"
        )
        print(
            f"  after land2+floor vs {need} cows ${extra_cows}: "
            f"{'FITS all-14 after land2' if leftover_at_land2 - LAND_FLOOR >= extra_cows else 'DOES NOT FIT all-14 after land2'}"
        )
    d11_cash = d11e.get("money", 0)
    owned_d11 = d11e.get("owned", 0)
    need_d11 = max(0, TARGET_HERD - owned_d11)
    print(
        f"  EOD d11 ${d11_cash:.0f} herd={owned_d11} need {need_d11} more "
        f"(${need_d11 * COW_COST} cows): "
        f"{'could buy next morning' if d11_cash >= need_d11 * COW_COST + 50 else 'still short overnight for remaining'}"
    )
...
def main():
    ...
    for seed in seeds:
        print(f"running S3ne seed {seed}...", flush=True)
        print_seed(run_seed(seed, agent=S3NE))
