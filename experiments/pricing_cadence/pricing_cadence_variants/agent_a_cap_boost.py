...
# DIAGNOSTIC COUNTERFACTUAL (Task 5, pricing_cadence_v1_2_mechanism_audit):
# A's exact selling logic, unchanged, with every per-turn cap simply
# doubled - a deliberately simple, round, UNOPTIMIZED multiplier, not a
# tuned value. No cadence/urgency/pressure logic exists in this file at
# all; should_sell() and its static SELL_PRICE_THRESHOLDS are identical
# to agent_a.py. Purpose: isolate how much of D's gain over A is
# reproducible by raw throughput alone, with no forecasting or pressure
# awareness whatsoever. NOT a candidate for integration.
MAX_SELL_PER_TURN = {
    "STRAWBERRY": 20,
    "MELON": 30,
    "WOOL": 10,
