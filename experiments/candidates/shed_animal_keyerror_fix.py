...
def decide_market_actions(
    ...
    for product, quantity in shed.items():
        # ISSUE #21 TRACK C EXPERIMENT 2 FIX: a bought-but-uncollected
        # animal (e.g. "SHEEP", "COW") is a valid shed key but not a market
        # product - main.py's own market_price() KeyErrors on it once one
        # lingers in the shed across a sell-decision turn. Confirmed by
        # direct reproduction (see docs/candidate_reports/track_c_large_farm.md,
        # Experiment 5). Caught by nikaangukia_meroni's top-level
        # except-Exception safety net, which discards the PICKUP action
        # too, so the same crash recurs every turn for the rest of the
        # season - a one-time bug becomes a permanent freeze.
    ...
    if shed_total >= SHED_FORCE_SELL_THRESHOLD:
        for product, quantity in shed.items():
            # Same fix as the main sell loop above - see its comment.
            if product not in MARKET_PARAMS:
                continue
