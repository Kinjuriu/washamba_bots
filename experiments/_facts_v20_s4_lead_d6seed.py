...
# After first land, NE empties are ~25. Cap held+NE STRAW so d6 plants
# still hit S3ne (>=4) without the 7-pack. v20 buys 3 that day; we hold 1.
POST_LAND_EARLY_STRAW_NEED = 4
...
def decide_animal_market_actions(
    ...
    # one day of feed only. Until first land: restock only when shed+carry
    # is empty (emergency), one day of feed — sell-reserve still uses 2×.
    unlocked = farm.get("unlocked_quadrants") or ["NW"]
...
def straw_early_restock_quantity(farm, private, day, board_size, hour=None):
    ...
    cap = STRAW_EARLY_RESTOCK_CAP
    if day == 6 and not pending_first_land(farm):
        # Land afternoon only: fill to NEED (1 held + 3 buy = d6 STRAW >= 4)
        # then stop. Do not count 25 NE empties as an 8-pack, and do not
        # starve d7–8. hour > 11 is the dribble that spent the leftover.
        if hour is not None and hour > 11:
            return 0
        wanted = max(0, POST_LAND_EARLY_STRAW_NEED - held)
    else:
        wanted = max(0, min(empty, cap) - held)
    if wanted <= 0:
        return 0
...
def fact30_wants_straw_seed(farm, private, day, board_size):
    ...
    if held > 0:
        return False
    if day == 6 and not pending_first_land(farm):
        # Early restock owns the land afternoon. Do not stockpile-refill
        # from NE empties (that was the 10-pack). d7+ habit unchanged.
        return False
...
def melon_opening_restock_quantity(farm, private, day):
    ...
    if have == 0 and held == 0:
        ...
        wheat_est = 4 * 25  # fact 36: day-0 wheat buy is 1× owned, not 2×
...
def decide_market_actions(
    ...
    # Fact 31 / 30: seed restock — bulk on unlock morning, else dribble cadence.
    ...
    elif (early := straw_early_restock_quantity(
        farm, private, day, board_size, hour=hour,
    )):
        ...
        # d6 after first land: early restock owns STRAW; do not stockpile
        # from 25 NE empties.
        block_d6_straw = day == 6 and not pending_first_land(farm)
        if (
            not block_d6_straw
            and has_empty_later_extra_for_occupant(
                farm, day, board_size, "STRAWBERRY"
            )
        ):
